...
namespace MexGrpcSampleConsoleApp
{
    class Program
    {
        ...
        {
            Console.WriteLine("RegisterClientWithAuthWrongDevName Test Case");
    ...
    class MexGrpcLibApp
    {
        ...
        public void RunSampleFlow()
        {
            ...
            psi.Arguments = "-appname automation_api_auth_app -appvers 1.0 -devname automation_leon -privkeyfile " + pubkey;
            ...
            {
                if (regReplyError.Status.Detail == "failed to verify token - token developer mismatch")
                {
                    Console.WriteLine("AuthToken Incorrect!");
                    Console.WriteLine("Register Client With Auth Wrong Dev Name Return: " + regReplyError.Status.Detail);
                    Environment.Exit(1);
                }
                else
                {
                    Console.WriteLine("Register Client With Auth Wrong Dev Name Failed Return: " + regReplyError.Status.Detail);
                    Environment.Exit(1);
                }
