...
namespace MexGrpcSampleConsoleApp
{
    class Program
    {
        ...
        {
            Console.WriteLine("Get Connection Workflow GRPC Test Case");
    ...
    class MexGrpcLibApp
    {
        ...
        public async Task RunSampleFlow()
        {
            ...
            foreach (KeyValuePair<int, DistributedMatchEngine.AppPort> kvp in tcpAppPortDict)
            {
                if (kvp.Key == 2016)
                {
                    try
                    {
                        ...
                        Socket tcpConnection = await me.GetTCPConnection(findCloudletResponse, kvp.Value, kvp.Key, 10000);
            ...
            if (check == true)
            {
                Console.WriteLine("\nGetConnectionWorkflow GRPC Testcase Passed!!");
                Environment.Exit(0);
            }
            else
            {
                Console.WriteLine("\nGetConnectionWorkflow GRPC Testcase Failed!!");
                Environment.Exit(1);
            }
        }
