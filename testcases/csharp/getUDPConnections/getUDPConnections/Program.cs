...
namespace MexGrpcSampleConsoleApp
{
    class Program
    {
        ...
        {
            Console.WriteLine("Get UDP Connections GRPC Test Case");
    ...
    class MexGrpcLibApp
    {
        ...
        public async Task RunSampleFlow()
        {
            ...
            Console.WriteLine("UDP Port Testing\n");
            tcpAppPortDict = me.GetUDPAppPorts(findCloudletResponse);
            var check = false;
            foreach (KeyValuePair<int, DistributedMatchEngine.AppPort> kvp in tcpAppPortDict)
            {
                if (kvp.Key == 2015)
                {
                    try
                    {
                        ...
                        Socket udpConnection = await me.GetUDPConnection(findCloudletResponse, kvp.Value, kvp.Key, 10000);
                    ...
                    Console.WriteLine("\nUDP Connection Port " + kvp.Key + " finished.");
                }
            }
            if (check == true)
            {
                Console.WriteLine("\nGetUDPConnections GRPC Testcase Passed!!");
                Environment.Exit(0);
            }
            else
            {
                Console.WriteLine("\nGetUDPConnections GRPC Testcase Failed!!");
                Environment.Exit(1);
            }
        }
