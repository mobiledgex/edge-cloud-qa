...
ClientAppUsageMetrics - operator get with no cloudlet pools shall return error
   ...
   ${error}=  Run Keyword and Expect Error  *  Get Client App Usage Metrics  region=US  selector=latency  developer_org_name=${operator}  operator_org_name=tmus  limit=1  token=${op_manager_token}  use_defaults=${False}
   Should Contain  ${error}  code=400
   Should Contain  ${error}  {"message":"No non-empty CloudletPools to show"}

   ${error}=  Run Keyword and Expect Error  *  Get Client App Usage Metrics  region=US  selector=deviceinfo  developer_org_name=${operator}  operator_org_name=tmus  limit=1  token=${op_manager_token}  use_defaults=${False}
   Should Contain  ${error}  code=400
   Should Contain  ${error}  {"message":"No non-empty CloudletPools to show"}

   ${error}=  Run Keyword and Expect Error  *  Get Client App Usage Metrics  region=US  selector=x  developer_org_name=${operator}  operator_org_name=tmus  limit=1  token=${op_manager_token}  use_defaults=${False}
   Should Contain  ${error}  code=400
   Should Contain  ${error}  {"message":"No non-empty CloudletPools to show"}
...
ClientAppUsageMetrics - get with cluster not found shall return an empty list
   ...
   ${metrics}=  Get Client App Usage Metrics  region=US  selector=latency  limit=1  cloudlet_name=${cloudlet_name_openstack_metrics}  app_name=automation_api_app  app_version=1.0  operator_org_name=TDG  developer_org_name=mobiledgex  token=${token}  use_defaults=${False}
...
ClientAppUsageMetrics - get with appname not found shall return an empty list
   ...
   ${metrics}=  Get Client App Usage Metrics  region=US  selector=latency  limit=1  cloudlet_name=cloudlet_name_openstack_metrics  app_name=xx  app_version=1.0  operator_org_name=TDG  developer_org_name=mobiledgex  token=${token}  use_defaults=${False}
   Should Be Equal  ${metrics['data'][0]['Series']}        ${None}
   Should Be Equal  ${metrics['data'][0]['Messages']}      ${None}
