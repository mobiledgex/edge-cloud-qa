...
Metrics - Shall be able to get the last cloudlet ipusage metric on openstack
   ...
   ${metrics}=         MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  last=${last}
...
Metrics - Shall be able to get the last 5 cloudlet ipusage metrics on openstack
   ...
   ${metrics}=         MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  last=5
...
Metrics - Shall be able to get the last 100 cloudlet ipusage metrics on openstack
   ...
   ${metrics}=         MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  last=100
   log to console  ${metrics}
   log to console  ${metrics_influx}
...
Metrics - Shall be able to get all cloudlet ipusage metrics on openstack
   ...
   ${metrics}=  MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage
...
Metrics - Shall be able to request more ipusage metrics than exist on openstack
   ...
   ${metricsall}=  MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage
   ${num_readings_all}=  Get Length  ${metricsall['data'][0]['Series'][0]['values']}

   ${more_readings}=  Evaluate  ${num_readings_all} + 100
   ${metrics}=  MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  last=${more_readings}
...
Metrics - Shall be able to get the cloudlet ipusage metrics with starttime on openstack
   ...
   ${metricspre}=  MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  last=1
   ...
   ${metrics}=  MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  start_time=${start_date}
...
Metrics - Shall be able to get the cloudlet ipusage metrics with endtime on openstack
   ...
   ${metrics}=  MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  end_time=${end_date}
   log to console  ${metrics['data'][0]}
...
Metrics - Shall be able to get the cloudlet ipusage metrics with starttime=lastrecord on openstack
   ...
   ${metrics}=  MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  start_time=${metricspre['data'][0]['Series'][0]['values'][0][0]}
   log to console  ${metrics}
   @{datesplit_first}=  Split String  ${metrics['data'][0]['Series'][0]['values'][0][0]}  .
   @{datesplit_last}=   Split String  ${metrics['data'][0]['Series'][0]['values'][-1][0]}  .
   ${epoch_first}=  Convert Date  ${datesplit_first[0]}  result_format=epoch  date_format=%Y-%m-%dT%H:%M:%S
...
Metrics - Shall be able to get the cloudlet ipusage metrics with endtime=lastrecord on openstack
   ...
   # get readings and 1st and last timestamp
   ${metrics}=  MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  end_time=${metricspre['data'][0]['Series'][0]['values'][0][0]}
   log to console  ${metrics}
   @{datesplit_first}=  Split String  ${metrics['data'][0]['Series'][0]['values'][0][0]}  .
   @{datesplit_last}=   Split String  ${metrics['data'][0]['Series'][0]['values'][-1][0]}  .
   ${epoch_first}=  Convert Date  ${datesplit_first[0]}  result_format=epoch  date_format=%Y-%m-%dT%H:%M:%S
...
Metrics - Shall be able to get the cloudlet ipusage metrics with starttime > endtime on openstack
   ...
   # get readings and with starttime in the future
   ${metrics}=  MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  start_time=${start_date}  end_time=${end_date}
...
Metrics - Shall be able to get the cloudlet ipusage metrics with starttime and endtime and last on openstack
   ...
   # get readings with starttime and endtime
   ${metrics}=  MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  start_time=${start_date}  end_time=${end_date}  last=5
   @{datesplit_first}=  Split String  ${metrics['data'][0]['Series'][0]['values'][0][0]}  .
   @{datesplit_last}=   Split String  ${metrics['data'][0]['Series'][0]['values'][-1][0]}  .
   ${epoch_first}=  Convert Date  ${datesplit_first[0]}  result_format=epoch  date_format=%Y-%m-%dT%H:%M:%S
   ${epoch_last}=   Convert Date  ${datesplit_last[0]}  result_format=epoch  date_format=%Y-%m-%dT%H:%M:%S
...
Metrics - OperatorManager shall be able to get cloudlet ipusage metrics
   ...
   ${metrics}=         MexMasterController.Get Cloudlet Metrics  region=US  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_name=${operator}  selector=ipusage  last=5  token=${userToken}
   ${metrics_influx}=  MexInfluxDB.Get Influx Cloudlet IPUsage Metrics  cloudlet_name=${cloudlet_name_openstack_metrics}  condition=GROUP BY * ORDER BY DESC LIMIT 6  # last 5
   log to console  ${metrics}
   log to console  ${metrics_influx}
   log to console  ${metrics['data'][0]['Series'][0]['values'][0][0]}
...
Metrics - Shall be able to get the cloudlet ipusage metrics without cloudlet name on openstack
   ...
   ${metrics}=         MexMasterController.Get Cloudlet Metrics  region=US  operator_name=${operator}  selector=ipusage  last=${last}
   ${metrics_influx}=  MexInfluxDB.Get Influx Cloudlet IPUsage Metrics  operator_name=${operator}  condition=ORDER BY DESC LIMIT ${lastdb}  # last record
   log to console  ${metrics['data'][0]['Series']}
   log to console  ${metrics_influx}
