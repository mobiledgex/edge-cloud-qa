*** Settings ***
Documentation   k8s Cluster TCP Metrics
...
*** Test Cases ***
ClusterMetrics - k8s Dedicated Cluster TCP Metrics on openstack
   [Documentation]
   ...  request cluster TCP metrics with last=1
   ...  verify info is correct

   ${metrics}  ${metrics_influx}=  Get the last cluster metric on openstack   ${clustername_k8s}  ${cloudlet_name_openstack_metrics}  ${operator}  ${developer_name}  tcp
...
ClusterMetrics - last 5 k8s Dedicated Cluster TCP Metrics on openstack
   [Documentation]
   ...  request cluster TCP metrics with last=5
   ...  verify info is correct

   ${metrics}  ${metrics_influx}=  Get the last 5 cluster metrics on openstack     ${clustername_k8s}  ${cloudlet_name_openstack_metrics}  ${operator}  ${developer_name}  tcp
...
Setup
   ...
   ${clustername_k8s}=  set variable  k8sdedicated
   ${developer_name}=  Set Variable  testmonitor

   #${clustername_k8s}=   Set Variable  andycluster
   #${developer_name}=  Set Variable  automation_api

   Set Suite Variable  ${clustername_k8s}
