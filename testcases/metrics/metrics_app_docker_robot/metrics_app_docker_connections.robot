...
AppMetrics - Shall be able to get the last docker app Connections metric on openstack
   ...
   ${metrics}  ${metrics_influx}=  Get the last app metric on openstack   ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections
...
AppMetrics - Shall be able to get the last 5 docker app Connections metrics on openstack
   ...
   ${metrics}  ${metrics_influx}=  Get the last 5 app metrics on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections
...
AppMetrics - Shall be able to get the last 10 docker app Connections metrics on openstack
   [Documentation]
   ...  request cluster Connections metrics with last=10
   ...  verify info is correct

   ${metrics}  ${metrics_influx}=  Get the last 10 app metrics on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections
...
AppMetrics - Shall be able to get all docker app Connections metrics on openstack
   [Documentation]
...
AppMetrics - Shall be able to request more docker app Connections metrics than exist on openstack
   [Documentation]
   ...  request cluster Connections metrics with last=<greater than total number of metrics>
   ...  verify info is correct

   ${metrics}  ${metrics_influx}=  Get more app metrics than exist on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections
...
AppMetrics - Shall be able to get docker app Connections metrics with starttime on openstack
   [Documentation]
   ...  request cluster Connections metrics with starttime 
   ...  verify info is correct

   ${metrics}=  Get app metrics with starttime on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections

   Metrics Headings Should Be Correct  ${metrics}

   Connections Should Be In Range  ${metrics}

AppMetrics - Shall be able to get docker app Connections metrics with endtime on openstack
   [Documentation]
   ...  request cluster Connections metrics with endtime 
   ...  verify info is correct

   ${metrics}=  Get app metrics with endtime on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections

   Metrics Headings Should Be Correct  ${metrics}  

   Connections Should Be In Range  ${metrics}

AppMetrics - Shall be able to get the docker app Connections metrics with starttime=lastrecord on openstack
   [Documentation]
   ...
   ${metrics}  ${metrics_influx}=  Get app metrics with starttime=lastrecord on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections
...
AppMetrics - Shall be able to get the docker app Connections metrics with starttime > lastrecord on openstack
   ...
   Get app metrics with starttime > lastrecord on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections

AppMetrics - Shall be able to get the docker app Connections metrics with endtime=lastrecord on openstack
   [Documentation]
   ...
   ${metrics}=  Get app metrics with endtime=lastrecord on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections

   Metrics Headings Should Be Correct  ${metrics}

   Connections Should Be In Range  ${metrics}

AppMetrics - Shall be able to get the docker app Connections metrics with endtime = firstrecord on openstack
   [Documentation]
   ...  request cloudlet metrics with endtime = firstrecord 
   ...  verify empty list is returned

   Get app metrics with endtime = firstrecord on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections

AppMetrics - Shall be able to get the docker app Connections metrics with starttime > endtime on openstack
   [Documentation]
   ...  request cloudlet metrics with starttime > endtime
   ...  verify empty list is returned

   Get app metrics with starttime > endtime on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections

AppMetrics - Shall be able to get the docker app Connections metrics with starttime and endtime > lastrecord on openstack
   [Documentation]
   ...  request cloudlet metrics with starttime/endtime in the future
   ...  verify empty list is returned

   Get app metrics with starttime and endtime > lastrecord on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections

AppMetrics - Shall be able to get the docker app Connections metrics with starttime and endtime on openstack
   [Documentation]
   ...  request cluster Connections metrics with starttime and endtime on openstack
   ...  verify info is correct

   ${metrics}=  Get app metrics with starttime and endtime on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections

   Metrics Headings Should Be Correct  ${metrics}

   Connections Should Be In Range  ${metrics}

AppMetrics - Shall be able to get the docker app Connections metrics with starttime and endtime and last on openstack
   [Documentation]
   ...  request all cluster Connections metrics with starttime and endtime and last on openstack
   ...  verify info is correct

   ${metrics}=  Get app metrics with starttime and endtime and last on openstack     ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections

   Metrics Headings Should Be Correct  ${metrics}

   Connections Should Be In Range  ${metrics}

AppMetrics - DeveloperManager shall be able to get docker app Connections metrics
   [Documentation]
   ...  request the cluster Connections metrics as DeveloperManager
   ...  verify metrics are returned

   ${metrics}  ${metrics_influx}=  DeveloperManager shall be able to get app metrics  ${username}  ${password}  ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections
...
AppMetrics - DeveloperContributor shall be able to get docker app Connections metrics
   [Documentation]
   ...  request the cluster Connections metrics as DeveloperContributor
   ...  verify metrics are returned

   ${metrics}  ${metrics_influx}=  DeveloperContributor shall be able to get app metrics  ${username}  ${password}  ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections
...
AppMetrics - DeveloperViewer shall be able to get docker app Connections metrics
   [Documentation]
   ...  request the cluster Connections metrics as DeveloperViewer
   ...  verify metrics are returned

   ${metrics}  ${metrics_influx}=  DeveloperViewer shall be able to get app metrics  ${username}  ${password}  ${app_name}  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections
...
AppMetrics - Shall be able to get Docker app Connections metrics by version
   ...
   ${metrics1}   ${metrics_influx1}=  Get the last 5 app metrics on openstack with version     ${app_name}  1.0  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections 
   ${metrics2}   ${metrics_influx2}=  Get the last 5 app metrics on openstack with version     ${app_name}  2.0  ${app_name_influx}  ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  connections 
