...
CloudletUsageMetrics - get with numsamples and limit shall return error
   ...
   ${error}=  Run Keyword and Expect Error  *  Get Cloudlet Usage Metrics  region=US  selector=resourceusage  number_samples=1  limit=1  cloudlet_name=tmocloud-1  operator_org_name=dmuus  token=${token}  use_defaults=${False}
   Should Be Equal  ${error}  ('code=400', 'error={"message":"Only one of Limit or NumSamples can be specified"}')

   ${error}=  Run Keyword and Expect Error  *  Get Cloudlet Usage Metrics  region=US  selector=flavorusage  number_samples=1  limit=1  cloudlet_name=tmocloud-1  operator_org_name=dmuus  token=${token}  use_defaults=${False}
   Should Be Equal  ${error}  ('code=400', 'error={"message":"Only one of Limit or NumSamples can be specified"}')
