...
CloudletUsageMetrics - get with start age newer than end age shall return error
   ...
   ${error}=  Run Keyword and Expect Error  *  Get Cloudlet Usage Metrics  region=US  selector=flavorusage  cloudlet_name=tmocloud-1  operator_org_name=tmus  start_age=2m  end_age=3m  token=${token}  use_defaults=${False}
   Should Be Equal  ${error}  ('code=400', 'error={"message":"Start age must be before (older than) end age"}')
...
CloudletUsageMetrics - get with cloudlet not found shall return error
   ...
   ${error}=  Run Keyword and Expect Error  *  Get Cloudlet Usage Metrics  region=US  selector=flavorusage  limit=1  cloudlet_name=cloudlet_name_openstack_metrics  operator_org_name=TDG  token=${token}  use_defaults=${False}
   Should Contain  ${error}  ('code=400', 'error={"message":"Cloudlet does not exist"}')
