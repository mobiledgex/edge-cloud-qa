*** Settings ***
Documentation   K8s Dedicated Cluster CPU/Memory/Disk/Network/UDP/TCP Metrics
...
*** Test Cases ***
ClusterMetrics - Shall be able to get the last 5 k8s dedicated cluster CPU/Network/TCP/UDP/Mem/Disk metrics on openstack
   [Documentation]
   ...  request cluster CPU metrics with all selectors
   ...  verify info is correct

   ${metrics}=  Get the last 5 cluster metrics on openstack for multiple selectors     ${clustername_k8dedicated}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  cpu,network,tcp,udp,mem,disk
...
ClusterMetrics - Shall be able to get the last 5 k8s dedicated cluster wildcard metrics on openstack
   [Documentation]
