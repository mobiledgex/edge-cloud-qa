*** Settings ***
Documentation   Docker App Autocluster Network Metrics
...
*** Test Cases ***
AppMetrics - Shall be able to get all docker autoluster app Network metrics
   [Documentation]
   ...  - request all docker autocluster Network metrics
   ...  - verify info is correct

   [Tags]  ReservableCluster

   ${metrics}  ${metrics_influx}=  Get all app metrics on openstack     ${app_name}  ${app_name_influx}  ${realclustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  network
