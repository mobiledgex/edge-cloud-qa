...
ClusterMetrics - Shall be able to get all docker cluster autocluster Network metrics on openstack
   ...
   ${metrics}  ${metrics_influx}=  Get all cluster metrics on openstack     ${realclustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  network
