...
Setup
    ...
    ${config}=  Show Config
    ${numsamples_default}=  Set Variable  ${config['MaxMetricsDataPoints']}
    Set Suite Variable  ${numsamples_default}
    ...
    @{collection_intervals}=  Create List  11s  2m10s  3m10s
    ...
    Should Be Equal  ${cloudlet1.fqdn}  shared.${cloudlet_name}-tmus.${region_lc}.mobiledgex.net
    ...
    Should Be Equal  ${cloudlet2.fqdn}  shared.${cloudlet_name}-tmus.${region_lc}.mobiledgex.net
    ...
    Should Be Equal  ${cloudlet3.fqdn}  shared.${cloudlet2_name}-tmus.${region_lc}.mobiledgex.net
    ...
    Should Be Equal  ${lu.new_cloudlet.fqdn}  shared.${cloudlet2_name}-tmus.${region_lc}.mobiledgex.net
    ...
    Should Be Equal  ${lu3.new_cloudlet.fqdn}  shared.${cloudlet_name}-tmus.${region_lc}.mobiledgex.net
    ...
    Should Be Equal  ${lu4.new_cloudlet.fqdn}  shared.${cloudlet2_name}-tmus.${region_lc}.mobiledgex.net
...
Teardown Settings
   ...
   Update Settings  region=${region}  edge_events_metrics_collection_interval=${settings['edge_events_metrics_collection_interval']}  edge_events_metrics_continuous_queries_collection_intervals=@{collection_intervals}  token=${super_token}
...
Latency Values Should Be Correct
   ...
   [Arguments]  ${app_name}  ${metrics}  ${cloudlet}=${cloudlet_name}  ${raw}=${True}  ${time_diff}=${None}  ${numsamples}=${numsamples_default}
...
DeviceInfo Values Should Be Correct
   [Arguments]  ${metrics}  ${raw}=${True}  ${cloudlet}=${cloudlet_name}  ${time_diff}=${None}  ${numsamples}=${numsamples_default}
