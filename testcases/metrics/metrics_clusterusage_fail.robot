...
ClusterUsage - operator get with no cloudlet pools shall return error
   ...
   ${error}=  Run Keyword and Expect Error  *  Get Cluster Usage  region=US  developer_org_name=${operator}  operator_org_name=tmus  start_time=2006-01-02T15:04:05Z  end_time=2006-01-02T15:04:05Z  token=${op_manager_token}  use_defaults=${False}
   Should Contain  ${error}  code=400
...
ClusterUsage - get with cluster not found shall return an empty list
   ...
   ${metrics}=  Get Cluster Usage  region=US  start_time=2006-01-02T15:04:05Z  end_time=2006-01-02T15:04:05Z  cloudlet_name=${cloudlet_name_openstack_metrics}  operator_org_name=TDG  developer_org_name=mobiledgex  token=${token}  use_defaults=${False}
