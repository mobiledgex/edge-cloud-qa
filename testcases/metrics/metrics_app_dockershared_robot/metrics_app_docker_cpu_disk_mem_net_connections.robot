*** Settings ***
Documentation   docker sharedApp CPU/Memory/Disk/Network/Connections Metrics
...
*** Test Cases ***
AppMetrics - Shall be able to get the last docker shared app CPU/Network/Connections/Mem/Disk metrics on openstack
   [Documentation]
...
AppMetrics - Shall be able to get the last docker shared app wildcard metrics on openstack
   [Documentation]
...
#   ${clustername_vm}=   Set Variable  cluster-1616707297-578733-docker 
...
Disk Should Be In Range
  [Arguments]  ${metrics}
   ...
   # verify values
   FOR  ${reading}  IN  @{values}
     Should Be True               ${reading[20]} > 0 and ${reading[20]} <= 10000000000
   END
