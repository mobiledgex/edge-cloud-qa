...
ClientApiUsageMetrics - operator get with no cloudlet pools shall return error
   ...
   ${error}=  Run Keyword and Expect Error  *  Get Client Api Usage Metrics  region=US  selector=latency  developer_org_name=${operator}  operator_org_name=dmuus  limit=1  token=${op_manager_token}  use_defaults=${False}
   Should Contain  ${error}  code=400
   Should Contain  ${error}  {"message":"No non-empty CloudletPools to show"}

   ${error}=  Run Keyword and Expect Error  *  Get Client Api Usage Metrics  region=US  selector=deviceinfo  developer_org_name=${operator}  operator_org_name=dmuus  limit=1  token=${op_manager_token}  use_defaults=${False}
   Should Contain  ${error}  code=400
   Should Contain  ${error}  {"message":"No non-empty CloudletPools to show"}

   ${error}=  Run Keyword and Expect Error  *  Get Client Api Usage Metrics  region=US  selector=x  developer_org_name=${operator}  operator_org_name=dmuus  limit=1  token=${op_manager_token}  use_defaults=${False}
   Should Contain  ${error}  code=400
   Should Contain  ${error}  {"message":"No non-empty CloudletPools to show"}
...
ClientApiUsageMetrics - get with start age newer than end age shall return error
   ...
   ${error}=  Run Keyword and Expect Error  *  Get Client Api Usage Metrics  region=US  selector=api  limit=1  cloudlet_name=cloudlet  operator_org_name=operator  start_age=2  end_age=3  token=${token}  use_defaults=${False}
...
ClientApiUsageMetrics - get with cluster not found shall return an empty list
   ...
   ${metrics}=  Get Client Api Usage Metrics  region=US  selector=api  limit=1  cloudlet_name=${cloudlet_name_openstack_metrics}  app_name=automation_api_app  app_version=1.0  operator_org_name=GDDT  developer_org_name=mobiledgex  token=${token}  use_defaults=${False}
...
ClientApiUsageMetrics - get with appname not found shall return an empty list
   ...
   ${metrics}=  Get Client Api Usage Metrics  region=US  selector=api  limit=1  cloudlet_name=cloudlet_name_openstack_metrics  app_name=xx  app_version=1.0  operator_org_name=GDDT  developer_org_name=mobiledgex  token=${token}  use_defaults=${False}
   Should Be Equal  ${metrics['data'][0]['Series']}        ${None}
   Should Be Equal  ${metrics['data'][0]['Messages']}      ${None}
