...
ClientApiUsageMetrics - operator get with no cloudlet org shall return error
   ...
   Create Cloudlet  region=${region}  operator_org_name=dmuus
...
ClientApiUsageMetrics - operator get with no cloudlet pools shall return error
   ...
   ${error}=  Run Keyword and Expect Error  *  Get Client Api Usage Metrics  region=US  selector=api  developer_org_name=${operator}  operator_org_name=dmuus  limit=1  token=${op_manager_token}  use_defaults=${False}
   Should Contain  ${error}  code=400
