...
ClusterMetrics - Shall be able to get the last 5 docker cluster CPU/Network/TCP/UDP/Mem/Disk metrics on openstack
   ...
   ${metrics}=  Get the last 5 cluster metrics on openstack for multiple selectors     ${clustername_docker}  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  cpu,network,tcp,udp,mem,disk
