*** Settings ***
Documentation   Cluster TCP Metrics with no cluster name
...
ClusterMetrics - Shall be able to get the cluster TCP metrics with cloudlet/operator/developer only
   ...
   ${metrics}=  Get cluster metrics with cloudlet/operator/developer only  ${cloudlet_name_openstack_metrics}  ${operator_name_openstack}  ${developer_name}  tcp 
...
ClusterMetrics - Shall be able to get the cluster TCP metrics with cloudlet/developer only
   ...
   ${metrics}=  Get cluster metrics with cloudlet/developer only  ${cloudlet_name_openstack_metrics}  ${developer_name}  tcp 
...
ClusterMetrics - Shall be able to get the cluster TCP metrics with operator/developer only
   ...
   ${metrics}=  Get cluster metrics with operator/developer only  ${operator_name_openstack}  ${developer_name}  tcp 
...
ClusterMetrics - Shall be able to get all cluster TCP metrics with developer only
   ...
   ${metrics}=  Get all cluster metrics with developer only  ${developer_name}  tcp  1000
...
#   \  Should Be Equal  ${metrics['data'][0]['Series'][0]['values'][${index}][3]}  ${reading['tcp']}
