...
Partner shall be able to share zones
    ...
    Run Keyword And Expect Error  ('code=400', 'error={"message":"Federation with name \\\\\"${partner_federation_name}\\\\\" does not exist for self operator ID \\\\\"TDG\\\\\""}')   Share FederatorZone  federation_name=${partner_federation_name}  zoneid=Zone-ABC  selfoperatorid=TDG  token=${partner_super_token}
