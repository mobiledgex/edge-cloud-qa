...
*** Test Cases ***
Web UI - User shall be able to update a Kubernetes App for EU Region with Auth Public Key
    [Documentation]
    ...  Create a new EU Kubernetes App 
    ...  UpdateApp to add Auth Public Key

    Add New App  region=EU  app_name=${app_name}  developer_name=${developer_name}  deployment_type=kubernetes  access_ports=tcp:2015 

    MexConsole.Update App  auth_public_key=${app_key}
    Sleep  5s
...
Web UI - User shall be able to update a Docker App for EU Region with Auth Public Key
    [Documentation]
