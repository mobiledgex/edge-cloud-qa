...
${app_name}  someapplicationAuth   #has to match crm process startup parms
${developer_name}  AcmeAppCo
${app_version}  1.0

*** Test Cases ***
RegisterClient - request with auth shoudld return proper JWT
   ${token}=  Generate Auth Token  app_name=${app_name}  app_version=${app_version}  developer_name=${developer_name}
   
   Register Client	app_name=${app_name}  app_version=${app_version}  developer_name=${developer_name}  auth_token=${token}
