...
PlatformFindCloudlet - request shall return cloudlet without allianceorg
    ...
    ${appinst_1}=   Create App Instance  app_name=${app_name_default}  region=${region}  cloudlet_name=${cloudlet_name}dmuus    operator_org_name=dmuus  cluster_instance_name=autocluster
...
Setup
    ...
    Create App  region=${region}  access_ports=tcp:1  official_fqdn=${platos_uri}  #permits_platform_apps=${True}
    ${dmuus_appinst}=            Create App Instance  region=${region}  cloudlet_name=${dmuus_cloudlet_name}  operator_org_name=${dmuus_operator_name}  cluster_instance_name=autocluster
