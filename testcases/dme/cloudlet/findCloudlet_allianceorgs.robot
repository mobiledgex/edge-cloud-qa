...
FindCloudlet - request shall return cloudlet without allianceorg
    ...
    ...  - send findCloudlet with tmus and coord closer to tmus
    ...
    ${appinst_1}=   Create App Instance    region=${region}  developer_org_name=${developer_org_name_automation}  cloudlet_name=${cloudlet_name}tmus    operator_org_name=tmus  cluster_instance_name=autocluster  #developer_name=MobiledgeX
    ...
    ${cloudlet}=  Find Cloudlet  carrier_name=tmus  latitude=32  longitude=-91
...
FindCloudlet - request shall return cloudlet with allianceorg
    ...
    ...  - CreateCloudlet packet with tmus alliance org
    ...  - CreateAppinst on both cloudlets
    ...  - send findCloudlet with tmus and coord closer to packet
    ...
    log to console  ${appinst_1}
    Register Client  #developer_name=MobiledgeX
    ${cloudlet}=  Find Cloudlet  carrier_name=tmus  latitude=35  longitude=-94
...
FindCloudlet - request shall return updated cloudlet with allianceorg
    ...
    Create Cloudlet  region=${region}   cloudlet_name=${cloudlet_name}tmus    operator_org_name=tmus    latitude=${cloudlet_lat1}  longitude=${cloudlet_long1}
    Create Cloudlet  region=${region}   cloudlet_name=${cloudlet_name}packet  operator_org_name=packet  latitude=${cloudlet_lat2}  longitude=${cloudlet_long2}
    ${appinst_1}=   Create App Instance    region=${region}  developer_org_name=${developer_org_name_automation}  cloudlet_name=${cloudlet_name}tmus    operator_org_name=tmus  cluster_instance_name=autocluster 
    ...
    Register Client
    ${cloudlet}=  Find Cloudlet  carrier_name=tmus  latitude=35  longitude=-95
    Should Be Equal As Numbers  ${cloudlet.status}  1  #FIND_FOUND
    Should Be Equal             ${cloudlet.fqdn}  ${appinst_1['data']['uri']}
    Should Be Equal As Numbers  ${cloudlet.cloudlet_location.latitude}   ${cloudlet_lat1}
    Should Be Equal As Numbers  ${cloudlet.cloudlet_location.longitude}  ${cloudlet_long1}
...
FindCloudlet - request shall return cloudlet with allianceorg on both cloudlets
    ...
    ...  - send findCloudlet with tmus and coord closer to packet
    ...  - verify packet appinst is returned
    ...  - send findCloudlet with packet and coord closer to tmus
    ...  - verify tmus appinst is returned

    [Tags]  AllianceOrg

    ${allianceorgs_tmus}=  Create List  tmus
    ${allianceorgs_packet}=  Create List  packet
    Create Cloudlet  region=${region}   cloudlet_name=${cloudlet_name}tmus    operator_org_name=tmus    latitude=${cloudlet_lat1}  longitude=${cloudlet_long1}  alliance_org_list=${allianceorgs_packet}
    Create Cloudlet  region=${region}   cloudlet_name=${cloudlet_name}packet  operator_org_name=packet  latitude=${cloudlet_lat2}  longitude=${cloudlet_long2}  alliance_org_list=${allianceorgs_tmus}
...
FindCloudlet - request shall return cloudlet with multiple allianceorg
    ...
    ...  - send findCloudlet with tmus and coord closer to packet and verify packet appinst is returned
    ...  - send findCloudlet with att and coord closer to packet and verify packet appinst is returned
    ...  - send findCloudlet with TDG and coord closer to packet and verify packet appinst is returned
    ...  - send findCloudlet with TDG and coord closer to TDG and verify TDG appinst is returned

    [Tags]  AllianceOrg

    ${allianceorgs}=  Create List  tmus  att  TDG
    Create Cloudlet  region=${region}   cloudlet_name=${cloudlet_name}tmus    operator_org_name=tmus    latitude=${cloudlet_lat1}  longitude=${cloudlet_long1} 
    Create Cloudlet  region=${region}   cloudlet_name=${cloudlet_name}packet  operator_org_name=packet  latitude=${cloudlet_lat2}  longitude=${cloudlet_long2}  alliance_org_list=${allianceorgs}
    Create Cloudlet  region=${region}   cloudlet_name=${cloudlet_name}tdg  operator_org_name=TDG  latitude=${cloudlet_lat3}  longitude=${cloudlet_long3}
    ...
    ${appinst_4}=   Create App Instance    region=${region}  developer_org_name=${developer_org_name_automation}  cloudlet_name=${cloudlet_name}tdg  operator_org_name=TDG  cluster_instance_name=autocluster
    ...
    Register Client
    ${cloudlet}=  Find Cloudlet  carrier_name=TDG  latitude=35  longitude=-91
    Should Be Equal As Numbers  ${cloudlet.status}  1  #FIND_FOUND
    Should Be Equal             ${cloudlet.fqdn}  ${appinst_2['data']['uri']}
    Should Be Equal As Numbers  ${cloudlet.cloudlet_location.latitude}   ${cloudlet_lat2}
    Should Be Equal As Numbers  ${cloudlet.cloudlet_location.longitude}  ${cloudlet_long2}
    ...
    Register Client
    ${cloudlet}=  Find Cloudlet  carrier_name=TDG  latitude=45  longitude=-91
    Should Be Equal As Numbers  ${cloudlet.status}  1  #FIND_FOUND
