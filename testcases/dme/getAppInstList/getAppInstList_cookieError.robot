*** Settings ***
Documentation  GetAppInstList - various cookie erors
...
GetAppInstList - request with invalid cookie of x should return 'token contains an invalid number of segments'
   [Documentation]
   ...  Send getAppInstList with session cookie of 'x'
   ...  verify 'token contains an invalid number of segments' error is received

   ${error_msg}=  Run Keyword And Expect Error  *  Get App Instance List  session_cookie=x

   Should Contain  ${error_msg}   status = StatusCode.UNAUTHENTICATED
   Should Contain  ${error_msg}   details = "token contains an invalid number of segments"

GetAppInstList - request with invalid cookie of x.x.x should return 'illegal base64 data at input byte 1' with invalid cookie
   [Documentation]
   ...  Send GetAppInstList with session cookie of 'x.x.x'
   ...  verify 'illegal base64 data at input byte 1' error is received

   ${error_msg}=  Run Keyword And Expect Error  *  Get App Instance List  session_cookie=x.x.x
...
GetAppInstList - request with expired cookie should return 'token is expired by'
   [Documentation]
   ...  Send GetAppInstList  with expired session cookie
   ...
   ${error_msg}=  Run Keyword And Expect Error  *  Get App Instance List  session_cookie=${expired_cookie}
