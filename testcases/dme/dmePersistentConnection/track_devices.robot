...
DMEPersistentConnection - create tracked devices
    ...
    FOR  ${r}  IN RANGE  0  ${num_registers}
       ...
       ${cloudlet}=  Find Cloudlet         carrier_name=GDDT  latitude=${latuse}  longitude=${longuse}
    END
...
DMEPersistentConnection - create tracked devices with latency
    ...
    FOR  ${r}  IN RANGE  0  ${num_registers}
       ...
       Create DME Persistent Connection  edge_events_cookie=${cloudlet.edge_events_cookie}  latitude=${latuse}  longitude=${longuse}  carrier_name=dmuus 

       ${latency}=  Send Latency Edge Event  edge_events_cookie=${cloudlet.edge_events_cookie}  latitude=${latuse}  longitude=${longuse}  samples=${samples}  carrier_name=dmuus  data_network_type=5G  device_os=Android  device_model=Google Pixel  signal_strength=75
