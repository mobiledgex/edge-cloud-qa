...
DMEPersistentConnection - Latency edge event with device info shall return statistics
    ...
    Create DME Persistent Connection  edge_events_cookie=${cloudlet.edge_events_cookie}  latitude=36  longitude=-96  carrier_name=tmus  data_network_type=6G  device_os=Android  device_model=Google Pixel  signal_strength=65

    ${latency}=  Send Latency Edge Event  edge_events_cookie=${cloudlet.edge_events_cookie}  latitude=36  longitude=-96  samples=${samples}  carrier_name=tmus  data_network_type=6G  device_os=Android  device_model=Google Pixel  signal_strength=75
