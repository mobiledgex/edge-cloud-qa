*** Settings ***
Documentation  GetLocation - various cookie erors
...
#
#    ${error_msg}=  Run Keyword And Expect Error  *  Get Location  carrier_name=andy 
#
...
GetLocation - request without cookie should return 'missing cookie'
    [Documentation]
    ...  Send GetLocation without a session cookie
    ...  verify 'missing cookie' error is received
	
   ${error_msg}=  Run Keyword And Expect Error  *  Get Location  use_defaults=${False}
...
GetLocation - request with invalid cookie of x should return 'token contains an invalid number of segments'
   [Documentation]
   ...  Send GetLocation with session cookie of 'x'
   ...  verify 'token contains an invalid number of segments' error is received

   ${error_msg}=  Run Keyword And Expect Error  *  Get Location  session_cookie=x

   Should Contain  ${error_msg}   status = StatusCode.UNAUTHENTICATED
   Should Contain  ${error_msg}   details = "token contains an invalid number of segments"

GetLocation - request with invalid cookie of x.x.x should return 'illegal base64 data at input byte 1' with invalid cookie
   [Documentation]
   ...  Send GetLocation with session cookie of 'x.x.x'
   ...  verify 'illegal base64 data at input byte 1' error is received

   ${error_msg}=  Run Keyword And Expect Error  *  Get Location  session_cookie=x.x.x

   Should Contain  ${error_msg}   status = StatusCode.UNAUTHENTICATED
   Should Contain  ${error_msg}   details = "illegal base64 data at input byte 1"

GetLocation - request with truncated cookie should return 'VerifyCookie failed: Invalid cookie, no key'
   [Documentation]
   ...  Send GetLocation with parital session cookie
   ...
   ${error_msg}=  Run Keyword And Expect Error  *  GetLocation  session_cookie=eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE1NDIzODg0OTQsImlhdCI6MTU0MjM4ODQzNCwiZGV2bmFtZSI6IkFjbWVBcHBDbyIsImFwcG5hbWUiOiJzb21lYXBwbGljYXRpb25BdXRoIiwiYXBwdmVycyI6IjEuMCJ9.rc7V12dgiYDforzBQrPh

   Should Contain  ${error_msg}   status = StatusCode.UNAUTHENTICATED
   Should Contain  ${error_msg}   details = "VerifyCookie failed: Invalid cookie, no key"

GetLocation - request with expired cookie should return 'token is expired by'
   [Documentation]
   ...  Send GetLocation with expired session cookie
   ...
   ${error_msg}=  Run Keyword And Expect Error  *  Get Location  session_cookie=${expired_cookie}
