...
class tc_platformfindcloudlet_ratelimit(unittest.TestCase):
    ...
    def send_platformfindcloudlet(self, rate=None):
        def send_message():
            ...
            self.get_platform = mex_dme.PlatformFindCloudletRequest(carrier_name='dmuus', client_token='xxxx')
    ...
    def platformfindcloudlet_tokenbucket_aboverate(self, target):
        ...
        for r in fail_list:
            ...
            if 'status = StatusCode.RESOURCE_EXHAUSTED' in r:
                if target == 'PerIp':
                    if 'details = "Request for /distributed_match_engine.MatchEngineApi/PlatformFindCloudlet rate limited, please retry later. Error is: Client exceeded api rate limit per ip. Exceeded rate of 70.25 requests per second."' in r:
                        correct_error = True
                elif target == 'AllRequests':
                    if 'details = "Request for /distributed_match_engine.MatchEngineApi/PlatformFindCloudlet rate limited, please retry later. Error is: Exceeded rate of 70.25 requests per second."' in r:
                        correct_error = True
