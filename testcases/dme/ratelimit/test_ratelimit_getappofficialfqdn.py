    ...
    def getappofficialfqdn_tokenbucket_aboverate(self, target):
        ...
        for r in fail_list:
            ...
            if 'status = StatusCode.RESOURCE_EXHAUSTED' in r:
                if target == 'PerIp':
                    if 'details = "Request for /distributed_match_engine.MatchEngineApi/GetAppOfficialFqdn rate limited, please retry later. Error is: Client exceeded api rate limit per ip. Exceeded rate of 70.25 requests per second."' in r:
                        correct_error = True
                elif target == 'AllRequests':
                    if 'details = "Request for /distributed_match_engine.MatchEngineApi/GetAppOfficialFqdn rate limited, please retry later. Error is: Exceeded rate of 70.25 requests per second."' in r:
                        correct_error = True
