...
developer_name = 'Samsung'
app_name = 'SamsungEnablingLayer'
app_version = '1.0'
operator = 'tmus'
...
class tc_getfqdnlist_ratelimit(unittest.TestCase):
    @classmethod
    def setUpClass(self):
    ...
    def set_leakybucket_flow(self, requests_per_second, target):
        ...
        self.mc.create_rate_limit_flow(region='US', api_name='GetFqdnList', api_endpoint_type='Dme', rate_limit_target=target, flow_algorithm='LeakyBucketAlgorithm', requests_per_second=requests_per_second)
    ...
    def set_tokenbucket_flow(self, requests_per_second, target):
        ...
        self.mc.create_rate_limit_flow(region='US', flow_settings_name=self.flow_name_token, api_name='GetFqdnList', api_endpoint_type='Dme', rate_limit_target=target, flow_algorithm='TokenBucketAlgorithm', requests_per_second=requests_per_second, burst_size=15)

    def set_maxreqs(self, max_requests, interval, target):
        print('setting maxreqs')
        self.mc.create_rate_limit_max_requests(region='US', api_name='GetFqdnList', api_endpoint_type='Dme', rate_limit_target=target, max_requests_algorithm='FixedWindowAlgorithm', max_requests=max_requests, interval=interval)

    def send_getfqdnlist(self, rate=None):
        def send_message():
            global fail_list
            ...
            for r in range(1, num_requests + 1):
                try:
                    result = self.dme.get_fqdn_list(self.getfqdnlist.request)
    ...
    def getfqdnlist_leakybucket(self, target):
        self.fail_count = 0
    ...
    def getfqdnlist_tokenbucket_belowrate(self, target):
        self.fail_count = 0
        self.success_count = 0
    ...
    def getfqdnlist_tokenbucket_aboverate(self, target):
        ...
        starttime = time.time()
        self.send_getfqdnlist(rate=100)
        endtime = time.time()
        req_rate = (num_requests * num_threads) / (endtime - starttime)
        print('numrequests', num_requests, 'numthreads', num_threads, 'start', starttime, 'end', endtime, 'diff', endtime - starttime, 'successcount', self.success_count, 'failcount', self.fail_count, 'rate', req_rate, 'reqs/s')
        ...
        for r in fail_list:
            ...
            if 'status = StatusCode.RESOURCE_EXHAUSTED' in r:
                if target == 'PerIp':
                    if 'details = "Request for /distributed_match_engine.MatchEngineApi/GetFqdnList rate limited, please retry later. Error is: Client exceeded api rate limit per ip. Exceeded rate of 70.25 requests per second."' in r:
                        correct_error = True
                elif target == 'AllRequests':
                    if 'details = "Request for /distributed_match_engine.MatchEngineApi/GetFqdnList rate limited, please retry later. Error is: Exceeded rate of 70.25 requests per second."' in r:
                        correct_error = True
            expect(correct_error == True, 'status code fail. got ' + r)
        assert_expectations()

    def getfqdnlist_maxreqs(self, target):
        self.fail_count = 0
        self.success_count = 0
    ...
    def getfqdnlist_leakybucket_maxreqs(self, target):
        self.fail_count = 0
        self.success_count = 0
    ...
    def getfqdnlist_tokenbucket_maxreqs(self, target):
        self.fail_count = 0
        self.success_count = 0
    ...
    # ECQ-3864
    def test_getfqdnlist_leakybucket_perip(self):
        # [Documentation] RateLimiting - DME shall ratelimit GetFqdnList with LeakyBucket PerIp
        # ...  - configure ratelimit GetFqdnList with LeakyBucket PerIp
        # ...  - send high rate of GetFqdnList messages and verify they are processaed at configured rate

        self.getfqdnlist_leakybucket('PerIp')

    # ECQ-3865
    def test_getfqdnlist_leakybucket_allrequests(self):
        # [Documentation] RateLimiting - DME shall ratelimit GetFqdnList with LeakyBucket AllRequests
        # ...  - configure ratelimit GetFqdnList with LeakyBucket AllRequests
        # ...  - send high rate of GetFqdnList messages and verify they are processed at configured rate

        self.getfqdnlist_leakybucket('AllRequests')

    # ECQ-3866
    def test_getfqdnlist_tokenbucket_belowrate_perip(self):
        ...
        self.getfqdnlist_tokenbucket_belowrate('PerIp')

    # ECQ-3867
    def test_getfqdnlist_tokenbucket_belowrate_allrequests(self):
        ...
        self.getfqdnlist_tokenbucket_belowrate('AllRequests')

    # ECQ-3868
    def test_getfqdnlist_tokenbucket_aboverate_perip(self):
        # [Documentation] RateLimiting - DME shall ratelimit GetFqdnList with TokenBucket PerIp
        # ...  - configure ratelimit GetFqdnList with TokenBucket PerIp
        # ...  - send rate of GetFqdnList messages above the configured rate and verify they fail with the correct error

        self.getfqdnlist_tokenbucket_aboverate('PerIp')

    # ECQ-3869
    def test_getfqdnlist_tokenbucket_aboverate_allrequests(self):
        # [Documentation] RateLimiting - DME shall ratelimit GetFqdnList with TokenBucket AllRequests
        # ...  - configure ratelimit GetFqdnList with TokenBucket AllRequests
        # ...  - send rate of GetFqdnList messages above the configured rate and verify they fail with the correct error

        self.getfqdnlist_tokenbucket_aboverate('AllRequests')

    # ECQ-3870
    def test_getfqdnlist_maxreqs_perip(self):
        # [Documentation] RateLimiting - DME shall ratelimit GetFqdnList with MaxRequests PerIp
        # ...  - configure ratelimit GetFqdnList with MaxRequests PerIp
        # ...  - send rate of GetFqdnList messages above the configured rate and verify they fail with the correct error

        self.getfqdnlist_maxreqs('PerIp')

    # ECQ-3871
    def test_getfqdnlist_maxreqs_allrequests(self):
        # [Documentation] RateLimiting - DME shall ratelimit GetFqdnList with MaxRequests AllRequests
        # ...  - configure ratelimit GetFqdnList with MaxRequests AllRequests
        # ...  - send rate of GetFqdnList messages above the configured rate and verify they fail with the correct error

        self.getfqdnlist_maxreqs('AllRequests')

    # ECQ-3872
    def test_getfqdnlist_leakybucket_maxreqs_perip(self):
        # [Documentation] RateLimiting - DME shall ratelimit GetFqdnList with LeakyBucket/MaxReqs PerIp
        # ...  - configure ratelimit GetFqdnList with LeakyBucket/MaxReqs PerIp
        # ...  - send high rate of GetFqdnList messages and verify they fail for MaxReqs limit

        self.getfqdnlist_leakybucket_maxreqs('PerIp')

    # ECQ-3873
    def test_getfqdnlist_leakybucket_maxreqs_allrequests(self):
        # [Documentation] RateLimiting - DME shall ratelimit GetFqdnList with LeakyBucket/MaxReqs AllRequests
        # ...  - configure ratelimit GetFqdnList with LeakyBucket/MaxReqs AllRequests
        # ...  - send high rate of GetFqdnList messages and verify they fail for MaxReqs limit

        self.getfqdnlist_leakybucket_maxreqs('AllRequests')

    # ECQ-3874
    def test_getfqdnlist_tokenbucket_maxreqs_perip(self):
        ...
        self.getfqdnlist_tokenbucket_maxreqs('PerIp')

    # ECQ-3875
    def test_getfqdnlist_tokenbucket_maxreqs_allrequests(self):
        ...
        self.getfqdnlist_tokenbucket_maxreqs('AllRequests')
