...
int main() {
    cout << "FindCloudletFail Test Case";
    cout << endl;
    ...
    int authToken = system(("genauthtoken -appname automation_api_auth_app -appvers 1.0 -devname automation_dev_org -privkeyfile " + pubkey + " >token.txt").c_str());
    ...
    try {
	  ...
	  if(tokenURI == registerClientReply.token_server_uri()){
	    ...
	    if (findCloudletReply.status() == 2) {
	      cout << "Test Case Pass! Location Found Status: " << findCloudletReply.status() << endl;
	      exit(0);
	    }
