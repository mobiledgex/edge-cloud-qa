...
Controller shall return error while creating kubernetes based cluster and IpAccessShared with name greater than 40 characters
    ...
    ${error}=  Run Keyword and Expect Error  *  Create Cluster Instance   region=${region}  cloudlet_name=${cloudlet_name_openstack}  operator_org_name=${operator_name_openstack}  cluster_name=${cluster_name_long}  deployment=kubernetes  ip_access=IpAccessShared  number_nodes=1
