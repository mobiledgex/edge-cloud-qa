...
#
# delete a cluster that does not exist
# verify fails with 'Key not found'
#
...
class tc(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.cluster = cluster_pb2.Cluster(
                                           key = cluster_pb2.ClusterKey(name = 'lustername')
                                          )
    ...
    def test_DeleteClusterNameNotFound(self):
        # [Documentation] Cluster - User shall recieve an error when deleting a cluster that does not exist
        # ... delete a cluster that does not exist
        # ... verify fails with 'Key not found'

        try:
            create_cluster_resp = self.cluster_stub.DeleteCluster(self.cluster)
        except grpc.RpcError as e:
            print('error', type(e.code()), e.details())
            expect_equal(e.code(), grpc.StatusCode.UNKNOWN, 'status code')
            expect_equal(e.details(), 'Key not found', 'error details')
        else:
