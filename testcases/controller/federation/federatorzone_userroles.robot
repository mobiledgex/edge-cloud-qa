...
OperatorContributor shall be able to create/view/delete federatorzone
    ...
    Create Cloudlet  region=${region}  operator_org_name=dmuus  cloudlet_name=${cloudlet_name}  platform_type=PlatformTypeFake  number_dynamic_ips=254  latitude=31  longitude=-91  env_vars=FAKE_RAM_MAX=4096000,FAKE_VCPUS_MAX=1000,FAKE_DISK_MAX=100000
    ...
    Create FederatorZone  operatorid=dmuus  countrycode=${selfcountrycode}  cloudlets=${cloudlets}  geolocation=50,50  region=${region}  zoneid=${federator_zone}  use_defaults=${False}  token=${op1_token}  auto_delete=${False}

    ${federatorzone_show}=  Show FederatorZone  operatorid=dmuus  zoneid=${federator_zone}  token=${op1_token}
    Should Not Be Empty  ${federatorzone_show}

    Delete FederatorZone  operatorid=dmuus  countrycode=${selfcountrycode}  zoneid=${federator_zone}  use_defaults=${False}  token=${op1_token}
...
OperatorViewer shall not be able to create/delete federatorzone
    ...
    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Create FederatorZone  operatorid=dmuus  countrycode=${selfcountrycode}  cloudlets=${cloudlets}  geolocation=50,50  region=${region}  zoneid=${federator_zone}  use_defaults=${False}  token=${op1_token}  auto_delete=${False}

    Create FederatorZone  operatorid=dmuus  countrycode=${selfcountrycode}  cloudlets=${cloudlets}  geolocation=50,50  region=${region}  zoneid=${federator_zone}  use_defaults=${False}

    ${federatorzone_show}=  Show FederatorZone  operatorid=dmuus  zoneid=${federator_zone}  token=${op1_token}
    Should Not Be Empty  ${federatorzone_show}

    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Delete FederatorZone  operatorid=dmuus  countrycode=${selfcountrycode}  zoneid=${federator_zone}  use_defaults=${False}  token=${op1_token}
...
DeveloperManager shall not be able to create/delete/view federatorzone
    ...
    ${dev1_token}=  Login  username=${dev_manager_user_automation}  password=${dev_manager_password_automation}
     ...
     Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Create FederatorZone  operatorid=dmuus  countrycode=${selfcountrycode}  cloudlets=${cloudlets}  geolocation=50,50  region=${region}  zoneid=${federator_zone}  use_defaults=${False}  token=${dev1_token}  auto_delete=${False}

    Create FederatorZone  operatorid=dmuus  countrycode=${selfcountrycode}  cloudlets=${cloudlets}  geolocation=50,50  region=${region}  zoneid=${federator_zone}  use_defaults=${False}

    ${federatorzone_show}=  Show FederatorZone  operatorid=dmuus  zoneid=${federator_zone}  token=${dev1_token}
    Should Be Empty  ${federatorzone_show}

    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Delete FederatorZone  operatorid=dmuus  countrycode=${selfcountrycode}  zoneid=${federator_zone}  use_defaults=${False}  token=${dev1_token}
