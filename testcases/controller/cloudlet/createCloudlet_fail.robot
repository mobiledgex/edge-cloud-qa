...
CreateCloudlet - create with trust policy on non-openstack shall return error
   ...
   Run Keyword and Expect Error  ('code=400', 'error={"message":"Trust Policy not supported on PLATFORM_TYPE_K8S_BARE_METAL"}')  Create Cloudlet  region=US  platform_type=PlatformTypeK8SBareMetal  operator_org_name=${oper}  latitude=1  longitude=1  number_dynamic_ips=1  trust_policy=${policy_return['data']['key']['name']}
...
Create Cloudlet with singlekubernetesclusterowner
   ...
   Run Keyword and Expect Error  ('code=200', 'error={"result":{"message":"Single kubernetes cluster owner can only be set on a single cluster platform","code":400}}')  Create Cloudlet  region=US  operator_org_name=GDDT  latitude=1  longitude=1  number_dynamic_ips=1  platform_type=${platform}  single_kubernetes_cluster_owner=${developer_org_name_automation}  vm_pool=${vmpool}  token=${token}
