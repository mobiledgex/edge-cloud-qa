...
CreateAutoScalePolicy - shall be able to create with min parms 
   ...
   ${policy_return}=  Create Autoscale Policy  region=US  token=${token}  policy_name=${policy_name}  developer_name=${developer_name}  min_nodes=1  max_nodes=2  scale_up_cpu_threshold=1  use_defaults=False
...
CreateAutoScalePolicy - shall be able to create with max values 
   ...
   ${policy_return}=  Create Autoscale Policy  region=US  token=${token}  policy_name=aasdfadfasdfasdfasfdadfasdfadsfadsfasdfasdffasdfdasafdsewrqwerqwerwerqerqerqw  developer_name=asdfafdafasfafadfadfadfafafafafafasfawerqwerqwerqwerqwrqrqqwqwrqwerwqrwq  min_nodes=9  max_nodes=10  scale_up_cpu_threshold=100  scale_down_cpu_threshold=99  trigger_time=100000000  use_defaults=False
   log to console  ${policy_return}
   ...
   Should Be Equal             ${policy_return['data']['key']['developer']}       asdfafdafasfafadfadfadfafafafafafasfawerqwerqwerqwerqwrqrqqwqwrqwerwqrwq 
...
CreateAutoScalePolicy - shall be able to create with min values
   ...
   ${policy_return}=  Create Autoscale Policy  region=US  token=${token}  policy_name=a  developer_name=b  min_nodes=1  max_nodes=2  scale_up_cpu_threshold=2  scale_down_cpu_threshold=1  trigger_time=1  use_defaults=False
   log to console  ${policy_return}

   Should Be Equal             ${policy_return['data']['key']['name']}            a 
   Should Be Equal             ${policy_return['data']['key']['developer']}       b 
   Should Be Equal As Numbers  ${policy_return['data']['min_nodes']}              1
   Should Be Equal As Numbers  ${policy_return['data']['max_nodes']}              2 
...
CreateAutoScalePolicy - shall be able to create with scaleup and scaledown 
   ...
   ${policy_return}=  Create Autoscale Policy  region=US  token=${token}  policy_name=${policy_name}  developer_name=${developer_name}  min_nodes=1  max_nodes=2  scale_up_cpu_threshold=2  scale_down_cpu_threshold=1  use_defaults=False
   log to console  ${policy_return}
...
CreateAutoScalePolicy - shall be able to create with all values
   ...
   ${policy_return}=  Create Autoscale Policy  region=US  token=${token}  policy_name=${policy_name}  developer_name=${developer_name}  min_nodes=5  max_nodes=8  scale_up_cpu_threshold=20  scale_down_cpu_threshold=10  trigger_time=10  use_defaults=False
   log to console  ${policy_return}
