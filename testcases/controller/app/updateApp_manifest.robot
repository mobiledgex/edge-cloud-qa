...
# not sure what this test is. I didnt find it in jira
...
# ECQ-2810
UpdateApp - user shall not be able to update ports only with k8s user manifest
    [Documentation]
    ...  - create a k8s app with tcp access port and tcp manifest
    ...  - verify app ports and manifest are correct
...
# ECQ-3478
UpdateApp - user shall be able to update ports only with vm user manifest
    [Documentation]
    ...  - create a vm app with tcp access port and manifest
    ...  - verify app ports and manifest are correct
    ...  - update the app with port only
    ...  - verify ports are updated and manifest remains

    ${app1}=  Create App  access_ports=${access_ports_tcp}  deployment=vm  image_type=ImageTypeQcow  image_path=${qcow_centos_image}  deployment_manifest=${server_ping_threaded_cloudconfig}

    Should Be Equal     ${app1.access_ports}         ${access_ports_tcp}

    ${app2}=  Update App  access_ports=${access_ports_udp}

    Should Be Equal  ${app2.access_ports}         ${access_ports_udp}

    Should Be Equal  ${app1.deployment_manifest}  ${app2.deployment_manifest}

# ECQ-3479
UpdateApp - user shall be able to update ports only with docker user manifest
    [Documentation]
    ...  - create a docker app with tcp access port and manifest
    ...  - verify app ports and manifest are correct
    ...  - update the app with port only
    ...  - verify ports are updated and manifest remains

    ${app1}=  Create App  access_ports=${access_ports_tcp}  deployment=docker  image_type=ImageTypeDocker  image_path=${docker_image}  deployment_manifest=${manifest}
