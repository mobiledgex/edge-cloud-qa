...
# ECQ-2238
CreateAppInst - User shall be able to add TLS and non-TLS ports with cluster=docker/dedicated and app=docker/direct
   [Documentation]
   ...  deploy docker/dedicated clusterinst
   ...
   Create App  region=${region}  image_path=${docker_image}  access_ports=tcp:2000-2002:tls,tcp:999,tcp:2016:tls,tcp:8086:tls,udp:2016  image_type=ImageTypeDocker  deployment=docker  access_type=direct
   ${appInst}=  Create App Instance  region=${region}  cloudlet_name=${cloudlet_name}  operator_org_name=${operator_name}
