...
CreateCloudletPoolMember - create with same name shall return error
   ...
   Create Cloudlet Pool Member  region=US  token=${token}  operator_name=dmuus  cloudlet_name=tmocloud-1  

   ${error}=  Run Keyword And Expect Error  *   Create Cloudlet Pool Member  region=US  token=${token}  operator_name=dmuus  cloudlet_name=tmocloud-1
   
   Should Contain  ${error}   400
   Should Contain  ${error}   {"message":"CloudletPoolMember key {\\\\"pool_key\\\\":{\\\\"name\\\\":\\\\"${pool_name}\\\\"},\\\\"cloudlet_key\\\\":{\\\\"operator_key\\\\":{\\\\"name\\\\":\\\\"dmuus\\\\"},\\\\"name\\\\":\\\\"tmocloud-1\\\\"}} already exists"}
...
CreateCloudletPoolMember - create operator not found shall return error
   ...
   ${error}=  Run Keyword And Expect Error  *   Create Cloudlet Pool Member  region=US  token=${token}  operator_name=dmuusxxx  cloudlet_name=tmocloud-1
   
   Should Contain  ${error}   400
   Should Contain  ${error}   {"message":"Cloudlet key {\\\\"operator_key\\\\":{\\\\"name\\\\":\\\\"dmuusxxx\\\\"},\\\\"name\\\\":\\\\"tmocloud-1\\\\"} not found"}
...
CreateCloudletPoolMember - create cloudlet not found shall return error
   ...
   ${error}=  Run Keyword And Expect Error  *   Create Cloudlet Pool Member  region=US  token=${token}  operator_name=dmuus  cloudlet_name=tmocloud-1xx
   
   Should Contain  ${error}   400
   Should Contain  ${error}   {"message":"Cloudlet key {\\\\"operator_key\\\\":{\\\\"name\\\\":\\\\"dmuus\\\\"},\\\\"name\\\\":\\\\"tmocloud-1xx\\\\"} not found"}
