...
UpdateCloudletPool - update without pool name shall return error
   ...
   ${error}=  Run Keyword And Expect Error  *  Update Cloudlet Pool  region=US  operator_org_name=GDDT  token=${token}  use_defaults=False
...
UpdateCloudletPool - update with CloudletPool not found shall return error
   ...
   Should Contain   ${error}  code=400
   Should Contain   ${error}  error={"message":"CloudletPool key {\\\\"organization\\\\":\\\\"GDDT\\\\",\\\\"name\\\\":\\\\"xxautomationVMPool\\\\"} not found"}
