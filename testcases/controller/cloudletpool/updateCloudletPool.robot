...
UpdateCloudletPool - shall be able to update pool after adding/removing members
   ...
   ${organization}=  Set Variable  TDG

   @{cloudlet_list}=  Create List  automationBerlinCloudlet
   ${pool_return}=  Create Cloudlet Pool  region=${region}  operator_org_name=${organization}  cloudlet_list=${cloudlet_list}
   Should Be Equal  ${pool_return['data']['key']['name']}  ${pool_name}
   Should Be Equal  ${pool_return['data']['key']['organization']}  ${organization}
   Should Be Equal  ${pool_return['data']['cloudlets']}  ${cloudlet_list}
   ...
   Add Cloudlet Pool Member  region=${region}  operator_org_name=${organization}  cloudlet_name=automationFrankfurtCloudlet
   Add Cloudlet Pool Member  region=${region}  operator_org_name=${organization}  cloudlet_name=automationHamburgCloudlet
   Remove Cloudlet Pool Member  region=${region}  operator_org_name=${organization}  cloudlet_name=automationHamburgCloudlet

   Sleep  1s
 
   @{cloudlet_list_update}=  Create List  automationDusseldorfCloudlet  automationBerlinCloudlet
