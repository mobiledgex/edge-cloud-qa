...
DeveloperOrg part of cloudletpool can only access gpudrivers mapped to private cloudlets belonging to the same cloudletpool
   ...
   Create Cloudlet Pool  region=${region_eu}  token=${super_token}  cloudlet_pool_name=${pool1}  operator_org_name=TDG
   Add Cloudlet Pool Member  region=${region_eu}  token=${super_token}  cloudlet_pool_name=${pool1}  operator_org_name=TDG  cloudlet_name=automationDusseldorfCloudlet
   ...
   Create Cloudlet Pool Access Invitation  region=${region_eu}  token=${op1_token}  cloudlet_pool_name=${pool1}  cloudlet_pool_org_name=TDG  developer_org_name=${org_name1}  use_defaults=False
   Create Cloudlet Pool Access Response  region=${region_eu}  token=${tokendev1}  cloudlet_pool_name=${pool1}  cloudlet_pool_org_name=TDG  developer_org_name=${org_name1}  decision=accept  use_defaults=False
   ...
   Create Gpudriver  region=${region_eu}  gpudriver_org=TDG  builds_dict=${driver_details}  token=${op1_token}
   Create Gpudriver  region=${region_eu}  gpudriver_org=reportorg1  builds_dict=${driver_details}  token=${op2_token}

   ${gpudriver1}=  Show Gpudriver  region=${region_eu}  gpudriver_org=TDG  token=${tokendev1}
   ...
   Update Cloudlet  region=${region_eu}  operator_org_name=TDG  cloudlet_name=automationDusseldorfCloudlet  gpudriver_name=${gpudriver_name}  gpudriver_org=TDG  token=${op1_token}
   ...
   ${gpudriver2}=  Show Gpudriver  region=${region_eu}  gpudriver_org=TDG  token=${tokendev2}

   Should Be Empty  ${gpudriver1}
   Should Be Empty  ${gpudriver2}

   Update Cloudlet  region=${region_eu}  operator_org_name=TDG  cloudlet_name=automationDusseldorfCloudlet  gpudriver_name=${Empty}  token=${op1_token}
...
Setup
    ...
    Run Keyword and Ignore Error  Adduser Role  username=${op_manager_user_automation}  orgname=TDG  role=OperatorManager  token=${super_token}
