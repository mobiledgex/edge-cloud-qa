...
Operator shall be able to UpdateCloudlet using a gpudriver with no org
    ...
    ${gpudriver}=  Update Cloudlet  region=${region}  operator_org_name=TDG  cloudlet_name=${cloudlet_name}  gpudriver_name=${gpudriver_name}  gpudriver_org=${Empty}
    Should Be Equal  ${gpudriver['data']['gpu_config']['driver']['name']}  ${gpudriver_name}
    ${gpudriver}=  Update Cloudlet  region=${region}  operator_org_name=TDG  cloudlet_name=${cloudlet_name}  gpudriver_name=${Empty}
...
OperatorManager of Org A cannot create/update/delete gpudriver of Operator Org B
    ...
    &{properties}=  Create Dictionary  FPS=10
    Create Gpudriver  region=${region}  gpudriver_org=TDG2

    ${tokenop}=  Login  username=op_manager_automation  password=${op_manager_password_automation}

    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Delete Gpudriver  region=${region}  gpudriver_org=TDG2  token=${tokenop}
    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Update Gpudriver  region=${region}  gpudriver_org=TDG2  properties=${properties}  token=${tokenop}
    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Create Gpudriver  region=${region}  gpudriver_org=TDG2  gpudriver_name=${gpudriver_name}1  token=${tokenop}
...
Developer can view/download gpudriver mapped to public cloudlet
    ...
    ${gpudriver}=  Show Gpudriver  region=${region}  gpudriver_org=TDG  token=${tokendev}
    Length Should Be   ${gpudriver[0]['data']['builds']}  1
    Should Be Equal  ${gpudriver[0]['data']['properties']['FPS']}  10

    ${buildurl}=  GetBuildUrl Gpudriver  region=${region}  gpudriver_org=TDG  build_name=${gpudriver_build}  token=${tokendev}
    Should Contain  ${buildurl['build_url_path']}  storage.googleapis.com
    Should Be Equal  ${buildurl['validity']}  20m0s

    Update Cloudlet  region=${region}  operator_org_name=TDG  cloudlet_name=${cloudlet_name}  gpudriver_name=${Empty}  token=${super_token}
