...
Operator shall be able to UpdateCloudlet using a gpudriver with no org
    ...
    Adduser Role  username=${usernameop_epoch}  orgname=TDG  role=OperatorContributor
    ...
    ${gpudriver}=  Update Cloudlet  region=${region_eu}  operator_org_name=TDG  cloudlet_name=${cloudlet_name}  gpudriver_name=${gpudriver_name}  gpudriver_org=${Empty}
    Should Be Equal  ${gpudriver['data']['gpu_config']['driver']['name']}  ${gpudriver_name}
    ${gpudriver}=  Update Cloudlet  region=${region_eu}  operator_org_name=TDG  cloudlet_name=${cloudlet_name}  gpudriver_name=${Empty}
...
OperatorManager of Org A shall not be able to UpdateCloudlet using gpudriver of Org B
    ...
    &{properties}=  Create Dictionary  FPS=10
    ${gpudriver}=  Create Gpudriver  region=US  gpudriver_org=tmus  builds_dict=${driver_details}  properties=${properties}  token=${tokenop1}

    ${error_msg}=  Run Keyword and Expect Error  *  Update Cloudlet  region=US  operator_org_name=packet  cloudlet_name=packet-qaregression  gpudriver_name=${gpudriver_name}  gpudriver_org=tmus  token=${tokenop}
...
OperatorContributor shall be able to create/update/delete/addbuild/removebuild to a gpudriver
    ...
    ${gpudriver}=  Addbuild Gpudriver  region=US  gpudriver_org=tmus  build_name=${gpudriver_build}1  build_driverpath=${driverpath1}  build_os=Linux  build_md5sum=${md5sum1}  build_driverpathcreds=apt:mobiledgex  build_kernelversion=${kernelversion1}  token=${tokenop}
...
OperatorViewer shall not be able to create/update/delete/addbuild/removebuild to a gpudriver
    ...
    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Create Gpudriver  region=US  gpudriver_org=tmus  builds_dict=${driver_details}

    ${tokenopm}=  Login  username=op_manager_automation  password=${op_manager_password_automation}
    Create Gpudriver  region=US  gpudriver_org=tmus  builds_dict=${driver_details}  token=${tokenopm}
    ${gpudriver}=   Show Gpudriver  region=US  gpudriver_org=tmus  token=${tokenopv}
    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Update Gpudriver  region=US  gpudriver_org=tmus  properties=${properties}  token=${tokenopv}
    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Delete Gpudriver  region=US  gpudriver_org=tmus  token=${tokenopv}
    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Addbuild Gpudriver  region=US  gpudriver_org=tmus  build_name=${gpudriver_build}1  build_driverpath=${driverpath}  build_os=Linux  build_md5sum=${md5sum}  build_driverpathcreds=apt:mobiledgex  build_kernelversion=4.15.0-143-generic  token=${tokenopv}
    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Removebuild Gpudriver  region=US  gpudriver_org=tmus  build_name=${gpudriver_build}  token=${tokenopv}
...
OperatorManager of Org A cannot create/update/delete gpudriver of Operator Org B
    ...
    &{properties}=  Create Dictionary  FPS=10
    Create Gpudriver  region=${region_eu}  gpudriver_org=TDG2

    ${tokenop}=  Login  username=op_manager_automation  password=${op_manager_password_automation}

    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Delete Gpudriver  region=${region_eu}  gpudriver_org=TDG2  token=${tokenop}
    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Update Gpudriver  region=${region_eu}  gpudriver_org=TDG2  properties=${properties}  token=${tokenop}
    Run Keyword and Expect Error  ('code=403', 'error={"message":"Forbidden"}')  Create Gpudriver  region=${region_eu}  gpudriver_org=TDG2  gpudriver_name=${gpudriver_name}1  token=${tokenop}
...
Developer can view/download gpudriver mapped to public cloudlet
    ...
    &{properties}=  Create Dictionary  FPS=10
    ${gpudriver}=  Create Gpudriver  region=${region_eu}  gpudriver_org=TDG  builds_dict=${driver_details}  properties=${properties}
    ...
    ${gpudriver}=  Show Gpudriver  region=${region_eu}  gpudriver_org=TDG  token=${tokendev}
    Length Should Be   ${gpudriver[0]['data']['builds']}  1
    Should Be Equal  ${gpudriver[0]['data']['properties']['FPS']}  10

    ${buildurl}=  GetBuildUrl Gpudriver  region=${region_eu}  gpudriver_org=TDG  build_name=${gpudriver_build}  token=${tokendev}
    Should Contain  ${buildurl['build_url_path']}  storage.googleapis.com
    Should Be Equal  ${buildurl['validity']}  20m0s

    Update Cloudlet  region=${region_eu}  operator_org_name=TDG  cloudlet_name=${cloudlet_name}  gpudriver_name=${Empty}  token=${super_token}
