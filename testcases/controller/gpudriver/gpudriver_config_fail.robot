...
Controller throws error if adding build for Linux OS without providing kernel version
    ...
    ${error_msg}=  Run Keyword and Expect Error  *  Create Gpudriver  region=${region_eu}  gpudriver_org=GDDT  builds_dict=${driver_details}  properties=${properties}
    Should Contain  ${error_msg}  Kernel version is required for Linux build

    Create Gpudriver  region=${region_eu}  gpudriver_org=GDDT
    ${error_msg}=  Run Keyword and Expect Error  *  Addbuild Gpudriver  region=${region_eu}  gpudriver_org=GDDT  build_name=${gpudriver_build}  build_driverpath=${driverpath}  build_os=Linux  build_md5sum=${md5sum}  build_driverpathcreds=apt:mobiledgex    
    Should Contain  ${error_msg}  Kernel version is required for Linux build

    ${gpudriver}=  Show Gpudriver  region=${region_eu}  gpudriver_org=GDDT
...
Controller throws error if invalid md5 checksum is provided while adding build
    ...
    ${error_msg}=  Run Keyword and Expect Error  *  Create Gpudriver  region=${region_eu}  gpudriver_org=GDDT  builds_dict=${driver_details}
    ...
    Create Gpudriver  region=${region_eu}  gpudriver_org=GDDT
    ${error_msg}=  Run Keyword and Expect Error  *  Addbuild Gpudriver  region=${region_eu}  gpudriver_org=GDDT  build_name=${gpudriver_build}  build_driverpath=${driverpath}  build_os=Linux  build_md5sum=${md5sum}1  build_driverpathcreds=apt:mobiledgex  build_kernelversion=4.15.0-143-generic
    Should Contain  ${error_msg}  'code=200', 'error={"result":{"message":"Invalid md5sum specified, expected md5sum 4acc71e8d54bfa92d7f12debf7e0a3a9","code":400}}'
...
Controller throws error when driverpath contains package other than debian
    ...
    Create Gpudriver  region=${region_eu}  gpudriver_org=GDDT
    ${error_msg}=  Run Keyword and Expect Error  *  Addbuild Gpudriver  region=${region_eu}  gpudriver_org=GDDT  build_name=${gpudriver_build}  build_driverpath=${package}  build_os=Linux  build_md5sum=${md5sum}  build_driverpathcreds=apt:mobiledgex  build_kernelversion=4.15.0-143-generic
    Should Contain  ${error_msg}  'code=200', 'error={"result":{"message":"Only supported file extension for Linux GPU driver is \\'.deb\\', given .run","code":400}}'
...
Controller throws error while adding build if debian package was built for older kernel version
    ...
    Create Gpudriver  region=${region_eu}  gpudriver_org=GDDT
    ${error_msg}=  Run Keyword and Expect Error  *  Addbuild Gpudriver  region=${region_eu}  gpudriver_org=GDDT  build_name=${gpudriver_build}  build_driverpath=${driverpath}  build_os=Linux  build_md5sum=${md5sum}  build_driverpathcreds=qa-regression:L2ihKxGzo3  build_kernelversion=4.15.0-143-generic
    Should Contain  ${error_msg}  'code=200', 'error={"result":{"message":"Driver package(\\\\"GDDT/${gpudriver_name}/${gpudriver_build}.deb\\\\") should have Linux Kernel dependency(\\\\"4.15.0-143-generic\\\\") specified as part of debian control file
...
Controller throws error while adding build if invalid credentials are provided
    ...
    Create Gpudriver  region=${region_eu}  gpudriver_org=GDDT
    ${error_msg}=  Run Keyword and Expect Error  *  Addbuild Gpudriver  region=${region_eu}  gpudriver_org=GDDT  build_name=${gpudriver_build}  build_driverpath=${driverpath}  build_os=Linux  build_md5sum=${md5sum}  build_driverpathcreds=apt:mobiledgex  build_kernelversion=4.15.0-143-generic
    Should Contain  ${error_msg}  'code=200', 'error={"result":{"message":"Failed to download GPU driver build ${driverpath}, Invalid credentials to access URL: ${driverpath}","code":400}}
...
ClusterInst/AppInst creation of gpu flavor on a cloudlet not mapped to GPU driver fails
    ...
    Update Cloudlet  region=${region_eu}  operator_org_name=GDDT  cloudlet_name=${cloudlet_name}  gpudriver_name=${Empty}

    ${error_msg}=  Run Keyword and Expect Error  *   Create Cluster Instance  region=${region_eu}  operator_org_name=GDDT  cloudlet_name=${cloudlet_name}  ip_access=IpAccessDedicated  deployment=docker  flavor_name=automation_gpu_flavor
    ...
    ${error_msg}=  Run Keyword and Expect Error  *  Create App Instance  region=${region_eu}  operator_org_name=GDDT  cloudlet_name=${cloudlet_name}  cluster_instance_name=autocluster1
...
Deletion of GpuDriver is use by Cloudlet fails
    ...
    Create Gpudriver  region=${region_eu}  gpudriver_org=GDDT  builds_dict=${driver_details}
    ...
    ${error_msg}=  Run Keyword and Expect Error  *  Delete Gpudriver  region=${region_eu}  gpudriver_org=GDDT
...
ClusterInst create on cloudlet mapped to incompatible GPU driver fails
    ...
    Create Gpudriver  region=${region_eu}  gpudriver_org=GDDT
    Addbuild Gpudriver  region=${region_eu}  gpudriver_org=GDDT  build_name=${gpudriver_build}  build_driverpath=${driverpath1}  build_os=Linux  build_md5sum=${md5sum1}  build_driverpathcreds=apt:mobiledgex  build_kernelversion=4.15.0-135-generic

    Update Cloudlet  region=${region_eu}  operator_org_name=GDDT  cloudlet_name=${gpu_cloudlet}  gpudriver_name=${gpudriver_name}  gpudriver_org=GDDT
    ${error_msg}=  Run Keyword and Expect Error  *   Create Cluster Instance  region=${region_eu}  operator_org_name=GDDT  cloudlet_name=${gpu_cloudlet}  ip_access=IpAccessDedicated  deployment=docker  flavor_name=automation_gpu_flavor
    Should Contain  ${error_msg}  'code=200', 'error={"result":{"message":"Encountered failures: Create failed: failed to install GPU drivers on cluster VM: Unable to find Linux GPU driver build for kernel version 4.15.0-169-generic

    Update Cloudlet  region=${region_eu}  operator_org_name=GDDT  cloudlet_name=${gpu_cloudlet}  gpudriver_name=nvidia-450  gpudriver_org=GDDT
