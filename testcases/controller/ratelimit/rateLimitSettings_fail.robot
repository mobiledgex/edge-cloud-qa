...
CreateRateLimitFlow - create with invalid parms shall return error 
   ...
   Run Keyword and Expect Error  ('code=400', 'error={"message":"Invalid JSON data: Unmarshal error: expected RateLimitTarget, but got string AllRequest for field \\\\"FlowRateLimitSettings.key.rate_limit_key.rate_limit_target\\\\", valid values are one of UnknownTarget, AllRequests, PerIp, PerUser, or 0, 1, 2, 3"}')  Create Rate Limit Flow  region=${region}  token=${token}  flow_settings_name=xx  api_name=yy  rate_limit_target=AllRequest  api_endpoint_type=Dme  flow_algorithm=TokenBucketAlgorithm  requests_per_second=5  burst_size=5  use_defaults=${False}

   # invalid api_endpoint_type
   Run Keyword and Expect Error  ('code=400', 'error={"message":"Invalid JSON data: Unmarshal error: expected ApiEndpointType, but got string Dm for field \\\\"FlowRateLimitSettings.key.rate_limit_key.api_endpoint_type\\\\", valid values are one of UnknownApiEndpointType, Dme, or 0, 1"}')  Create Rate Limit Flow  region=${region}  token=${token}  flow_settings_name=xx  api_name=yy  rate_limit_target=AllRequests  api_endpoint_type=Dm  flow_algorithm=TokenBucketAlgorithm  requests_per_second=5  burst_size=5  use_defaults=${False}

   # invalid flow algor
   Run Keyword and Expect Error  ('code=400', 'error={"message":"Invalid JSON data: Unmarshal error: expected FlowRateLimitAlgorithm, but got string xxx for field \\\\"FlowRateLimitSettings.settings.flow_algorithm\\\\", valid values are one of UnknownFlowAlgorithm, TokenBucketAlgorithm, LeakyBucketAlgorithm, or 0, 1, 2"}')  Create Rate Limit Flow  region=${region}  token=${token}  flow_settings_name=xx  api_name=yy  rate_limit_target=AllRequests  api_endpoint_type=Dme  flow_algorithm=xxx  requests_per_second=5  burst_size=5  use_defaults=${False}
   Run Keyword and Expect Error  ('code=400', 'error={"message":"Invalid JSON data: Unmarshal error: expected FlowRateLimitAlgorithm, but got value 99 for field \\\\"FlowRateLimitSettings.settings.flow_algorithm\\\\", valid values are one of UnknownFlowAlgorithm, TokenBucketAlgorithm, LeakyBucketAlgorithm, or 0, 1, 2"}')  Create Rate Limit Flow  region=${region}  token=${token}  flow_settings_name=xx  api_name=yy  rate_limit_target=AllRequests  api_endpoint_type=Dme  flow_algorithm=${99}  requests_per_second=5  burst_size=5  use_defaults=${False}
...
CreateRateLimitMaxRequests - create with invalid parms shall return error
   ...
   # fixed - EDGECLOUD-5481 ratelimitsettings with invalid flowalgorithm,apiendpointtype and ratelimittarget need better error message
   # invalid rate limit target
   Run Keyword and Expect Error  ('code=400', 'error={"message":"Invalid JSON data: Unmarshal error: expected RateLimitTarget, but got string AllRequest for field \\\\\"MaxReqsRateLimitSettings.key.rate_limit_key.rate_limit_target\\\\\", valid values are one of UnknownTarget, AllRequests, PerIp, PerUser, or 0, 1, 2, 3"}')  Create Rate Limit Max Requests  region=${region}  token=${token}  max_requests_settings_name=xx  api_name=yy  rate_limit_target=AllRequest  api_endpoint_type=Dme  max_requests_algorithm=FixedWindowAlgorithm  max_requests=11  interval=1m

   # invalid api_endpoint_type
   Run Keyword and Expect Error  ('code=400', 'error={"message":"Invalid JSON data: Unmarshal error: expected ApiEndpointType, but got string Dm for field \\\\\"MaxReqsRateLimitSettings.key.rate_limit_key.api_endpoint_type\\\\\", valid values are one of UnknownApiEndpointType, Dme, or 0, 1"}')  Create Rate Limit Max Requests  region=${region}  token=${token}  max_requests_settings_name=xx  api_name=yy  rate_limit_target=AllRequests  api_endpoint_type=Dm  max_requests_algorithm=FixedWindowAlgorithm  max_requests=11  interval=1m

   # invalid interval
   Run Keyword and Expect Error  ('code=400', 'error={"message":"Invalid JSON data: Unmarshal error: expected duration, but got string 1 for field \\\\\"MaxReqsRateLimitSettings.settings.interval\\\\\", valid values are 300ms, 1s, 1.5h, 2h45m, etc"}')  Create Rate Limit Max Requests  region=${region}  token=${token}  max_requests_settings_name=xx  api_name=yy  rate_limit_target=AllRequests  api_endpoint_type=Dme  max_requests_algorithm=FixedWindowAlgorithm  max_requests=11  interval=1
   Run Keyword and Expect Error  ('code=400', 'error={"message":"Invalid JSON data: Unmarshal error: expected duration, but got string m for field \\\\\"MaxReqsRateLimitSettings.settings.interval\\\\\", valid values are 300ms, 1s, 1.5h, 2h45m, etc"}')  Create Rate Limit Max Requests  region=${region}  token=${token}  max_requests_settings_name=xx  api_name=yy  rate_limit_target=AllRequests  api_endpoint_type=Dme  max_requests_algorithm=FixedWindowAlgorithm  max_requests=11  interval=m
