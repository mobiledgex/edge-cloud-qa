...
UpdateVMPool - update with VMPool not found shall return error
   ...
   Should Contain   ${error}  code=400
   Should Contain   ${error}  error={"message":"VMPool key {\\\\"organization\\\\":\\\\"GDDT\\\\",\\\\"name\\\\":\\\\"xxautomationVMPool\\\\"} not found"}
...
UpdateVMPool - update with invalid state shall return error
   ...
   Should Contain   ${error}  error={"message":"Invalid JSON data: Unmarshal error: expected VMState, but got string x for field \\\\\"VMPool.vms.state\\\\\", valid values are one of Free, InProgress, InUse, Add, Remove, Update, ForceFree, or 0, 1, 2, 3, 4, 5, 6
   ...
   Should Contain   ${error}  error={"message":"Invalid JSON data: Unmarshal error: expected VMState, but got string 99 for field \\\\\"VMPool.vms.state\\\\\", valid values are one of Free, InProgress, InUse, Add, Remove, Update, ForceFree, or 0, 1, 2, 3, 4, 5, 6
