...
CreateAutoProvPolicy - mcctl shall handle create failures
   ...
   [Template]   Fail Create Auto Provisioning Policy Via mcctl
      ...
      Error: parsing arg "cloudlets:.key.organization\=GDDT" failed: invalid argument: key "cloudlets:.key.organization" not found   name=${recv_name}  region=${region}  app-org=testmonitor  minactiveinstances=1  cloudlets:.key.organization=GDDT  cloudlets:0.key.name=automationSunnydaleCloudlet  cloudlets:1.key.organization=GDDT  cloudlets:1.key.name=automationParadiseCloudlet
