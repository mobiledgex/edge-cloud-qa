...
#   log to console  ${nodes}
   ...
   Hawkins CRM Should Exist  ${nodes}
...
DME Should Exist
   [Arguments]  ${nodes}
   # {"data":{"key":{"name":"dme-647c84dc48-dx88z","node_type":1,"cloudlet_key":{"operator_key":{"name":"GDDT"},"name":"automationBuckhornCloudlet"}},"notify_id":42,"build_master":"v1.0.2-24-g277eca5","build_head":"v1.0.2-24-g277eca5","hostname":"dme-647c84dc48-dx88z"}}
   ...
   \  ${operator_match}=  Run Keyword And Return Status  Should Be Equal            ${node['data']['key']['cloudlet_key']['operator_key']['name']}  GDDT
   \  ${cloudlet_match}=  Run Keyword And Return Status  Should Be Equal            ${node['data']['key']['cloudlet_key']['name']}  automationBuckhornCloudlet 
...
Hawkins CRM Should Exist
   [Arguments]  ${nodes}
   #{'key': {'name': 'automationHawkinsCloudlet', 'node_type': 2, 'cloudlet_key': {'operator_key': {'name': 'GDDT'}, 'name': 'automationHawkinsCloudlet'}}, 'notify_id': 107, 'build_master': 'v1.0.2-24-g277eca5', 'build_head': 'v1.0.2-24-g277eca5', 'hostname': 'automationHawkinsCloudlet'}

   ${found}=  Set Variable  ${False}

   : FOR  ${node}  IN  @{nodes}
   \  log to console  ${node['data']}
   \  ${name_match}=  Run Keyword And Return Status  Should Be Equal                ${node['data']['key']['name']}  automationHawkinsCloudlet 
...
#   \  Log to console  ${node['data']['key']['cloudlet_key']['operator_key']['name']}
   Run keyword if  ${found}==False  fail  Hawkins CRM Not Found
