*** Settings ***
Documentation  ratelimitsettingsmc failures
...
# ECQ-3738
CreateRateLimitFlowMC - create with duplicate shall return error
   [Documentation]
   ...  - send same CreateRateLimitFlowMC twice 
   ...  - verify error is returned
   ...
   Create MC Rate Limit Flow  api_name=yy  rate_limit_target=AllRequests  flow_algorithm=TokenBucketAlgorithm  requests_per_second=5  burst_size=5

   ${error}=  Run Keyword and Expect Error  *  Create MC Rate Limit Flow  api_name=yy  rate_limit_target=AllRequests  flow_algorithm=TokenBucketAlgorithm  requests_per_second=5  burst_size=5
   Should Be Equal  ${error}  ('code=400', 'error={"message":"FlowRateLimitSettings with FlowSettingsName ${flow_name} already exists"}')
