...
SSH to RootLB VM
  ...
  SSH to Iperf Client Side Docker Container and Run UDP Performance Test for 30s

  ${stdout}  ${stderr}  ${rc}=  Execute Command  sudo apt-get install -y iperf3    return_stderr=${True}  return_rc=${True}
  Should Be Equal As Integers	${rc}	0

  ${stdout}  ${stderr}  ${rc}=  Execute Command  iperf3 -c dockerserver.automationDusseldorfCloudlet.TDG.mobiledgex.net -u -b 0 -t 30  return_stderr=${True}  return_rc=${True}
  Should Be Equal As Integers	${rc}	0
  Log  ${stdout}
