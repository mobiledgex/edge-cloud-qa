...
RunConsole - shall return error with non-vm app
    ...
    ${error}=  Run Keyword And Expect Error  *  Run Console  region=US  app_name=automation_api_app  app_version=1.0  developer_org_name=${developer}  cluster_instance_name=autoclusterautomation  cluster_instance_developer_org_name=MobiledgeX  operator_org_name=dmuus  cloudlet_name=tmocloud-1  token=${token}  command=ls
