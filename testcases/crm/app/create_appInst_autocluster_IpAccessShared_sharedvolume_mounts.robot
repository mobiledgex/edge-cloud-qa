...
Shall be able to configure IpAccessDedicated k8s autocluster with shared volume mounts
    ...
    ${rootlb}=  Catenate  SEPARATOR=.  autocluster${time}  ${cloudlet_name_openstack_shared}  ${operator_name_openstack}  ${mobiledgex_domain}
    ${rootlb}=  Convert To Lowercase  ${rootlb}
    ...
    ${app_name_default}=  Get Default App Name
    log to console  ${app_name_default}
    Create App Instance  region=${region}  cloudlet_name=${cloudlet_name_openstack_shared}  operator_org_name=${operator_name_openstack}  cluster_instance_name=autocluster${time}  autocluster_ip_access=IpAccessDedicated  shared_volume_size=10
