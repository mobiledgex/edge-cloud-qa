...
User shall be able to create multiple VM/LB deployment on 2 cloudlets on same tenant
    ...
    Create Cloudlet  region=US  operator_org_name=${operator_name_openstack}  platform_type=PlatformTypeOpenstack  physical_name=buckhorn  number_dynamic_ips=4  latitude=50.73438  longitude=7.09549  env_vars=FLAVOR_MATCH_PATTERN=m4,MEX_EXT_NETWORK=external-network-02  infra_config_external_network_name=external-network-02
    ...
    ${app_inst1}=  Create App Instance  app_name=${app_name_default}1  cloudlet_name=automationBuckhornCloudlet  operator_org_name=${operator_name_openstack}  cluster_instance_name=dummycluster  region=${region}  use_thread=${True}
    Sleep  5s
    ...
    Create App Instance  app_name=${app_name_default}1  cloudlet_name=automationBuckhornCloudlet  operator_org_name=${operator_name_openstack}  cluster_instance_name=dummycluster  region=${region}
