...
${operator_name_openstack_fip}  TDG
${physical_name_openstack_fip}  frankfurt
...
CreateCloudlet - User shall be able to create a cloudlet with floating IP
   ...
   #  mcctl --addr https://console-qa.mobiledgex.net:443 --skipverify region CreateCloudlet region=EU cloudlet=automationFipCloudlet cloudlet-org=TDG location.latitude=1 location.longitude=1 numdynamicips=254 platformtype=PlatformTypeOpenstack physicalname=frankfurt envvar=MEX_EXT_NETWORK=automationFIPNetwork envvar=MEX_NETWORK_SCHEME=cidr=172.168.X.0/24,floatingipnet=automationFIPNetwork,floatingipsubnet=automationFIPSubnet,floatingipextnet=external-network-shared
