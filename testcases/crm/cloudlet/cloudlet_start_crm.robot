...
CreateCloudlet - User shall be able to create a cloudlet on Openstack Bonn
        ...
        Run Keyword and Ignore Error  Create GPU Driver  region=US  gpudriver_name=nvidia-450  gpudriver_org=TDG
        Run Keyword and Ignore Error  Add Build GPU Driver  region=US  gpudriver_name=nvidia-450  gpudriver_org=TDG  build_name=Linux-4.15.0-159-generic  build_driverpath=https://artifactory.mobiledgex.net/artifactory/binaries/nvidia-gpu-passthrough/nvidia-450_450.119.03-4.15.0-159-generic-1_amd64.deb  build_md5sum=a32661ce39bdc9d503a1a277ffb8f616  build_os=Linux  build_kernelversion=4.15.0-159-generic  build_driverpathcreds=angshuman:jaWbu2)8Hb{xB
        ...
        Create App Instance  region=US  app_name=automation-sdk-porttest  app_version=1.0  developer_org_name=${developer_org_name_automation}  cluster_instance_name=porttestcluster  cloudlet_name=${cloudlet_name_openstack_bonn}  operator_org_name=${operator_name_openstack_bonn}  flavor_name=${flavor_name_automation}
...
CreateCloudlet - User shall be able to create a fake cloudlet
        ...
        Run Keyword and Continue on Failure  Create Cloudlet  region=EU  operator_org_name=tmus  cloudlet_name=eucloud-1  platform_type=PlatformTypeFake  number_dynamic_ips=254  latitude=41.38879  longitude=2.15899  env_vars=FAKE_RAM_MAX=4096000,FAKE_VCPUS_MAX=1000,FAKE_DISK_MAX=100000
        ...
        Run Keyword and Ignore Error  Create App  region=US  app_name=${app_name_automation_trusted}  app_version=1.0  developer_org_name=${developer_org_name_automation}  default_flavor_name=${flavor_name_automation}  trusted=${True}
...
DeleteCloudlet - User shall be able to delete a fake cloudlet
        ...
        Cleanup Clusters and Apps  region=EU  cloudlet_name=eucloud-1  crm_override=IgnoreCrmAndTransientState
        Run Keyword and Continue on Failure  Delete Cloudlet  region=EU  operator_org_name=tmus  cloudlet_name=eucloud-1
