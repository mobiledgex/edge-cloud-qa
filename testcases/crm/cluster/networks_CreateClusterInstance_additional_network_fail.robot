...
Setup Shared Cluster With Add Net Openstack
    ...
    Log To Console  ${\n}Creating Cluster Instance
    ${cluster_info_os}=  Run Keyword And Expect Error  *  Create Cluster Instance  token=${super_token}  region=${region}  cloudlet_name=automationBuckhornCloudlet  operator_org_name=GDDT  ip_access=Shared  number_masters=1  number_nodes=${num_nodes_x}  deployment=kubernetes  developer_org_name=automation_dev_org  cluster_name=${cluster_name}  networks=${networks-net}  timeout=${timeout}  flavor_name=${flavor_name}  #flavor_name=${cluster_flavor_name}      Set Suite Variable  ${cluster_info}i
...
Create Network If Not Existing Openstack
    ...
    ${exists_err}  Set Variable  ('code=400', 'error={"message":"Network key {\\\\"cloudlet_key\\\\":{\\\\"organization\\\\":\\\\"GDDT\\\\",\\\\"name\\\\":\\\\"automationBuckhornCloudlet\\\\"},\\\\"name\\\\":\\\\"${net-vcd-packet-ctb}\\\\"} already exists"}\')
    ${show_return}=  Show Network  region=${region}  token=${super_token}  network_name=${networks-net}  cloudlet_name=automationBuckhornCloudlet  cloudlet_org=GDDT    #connection_type=${connect1}  route_list=${routelist1}

    ${exists_yes}=  Run Keyword If  ${exists_err}==${show_return}  Set Variable  ${False}
    ${exists_no}=   Run Keyword If  ${show_return}==[]  Set Variable  ${True}
    IF  '${exists_no}'=='${True}'
         ${net_created}=  Create Network  region=${region}  token=${super_token}  network_name=${networks-net}  cloudlet_name=automationBuckhornCloudlet  cloudlet_org=GDDT  connection_type=${connect1}  route_list=${routelist1}
         Should Be Equal  ${net_created['data']['key']['cloudlet_key']['name']}  automationBuckhornCloudlet
         Should Be Equal  ${net_created['data']['key']['cloudlet_key']['organization']}  GDDT
         Should Be Equal  ${net_created['data']['connection_type']}  ${connect1}
         Should Be Equal  ${net_created['data']['routes']}  ${routelist1}
         Should Be Equal  ${net_created['data']['key']['name']}  ${networks-net}
         Log To Console   ${networks-net} was created
