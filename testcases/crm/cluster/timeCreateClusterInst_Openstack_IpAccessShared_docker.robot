...
*** Test Cases ***
# ECQ-2698	
ClusterInst shall create single with IpAccessShared/docker on openstack
   [Documentation]
   ...
   ...  collect the time it takes to create the cluster and write it to a file
	...
	${FileData}=     Set Variable       ${testnum} Openstack Docker Shared Cluster Creation Time: ${epoch_total_time}\n
	Append To File   ${EXECDIR}/${FileName}     ${FileData}     
	Write Data   


# ECQ-2699
ClusterInst shall create 2 with IpAccessShared/docker on openstack
   [Documentation]
   ...  create 2 clusters on openstack with IpAccessShared and deploymenttype=docker
   ...  collect the time it takes to create the clusters and write it to a file
	...
	\  ${handle}=   Create Cluster Instance   cluster_name=${cluster_name}   cloudlet_name=${cloudlet_name_openstack}   operator_org_name=${operator_name_openstack}      flavor_name=${flavor_name}   number_nodes=0  number_masters=0   ip_access=IpAccessShared    deployment=docker     use_thread=${True}    del_thread=${True}
...
# ECQ-2700
ClusterInst shall create 4 with IpAccessShared/docker on openstack
   [Documentation]
   ...  create 4 clusters on openstack with IpAccessShared and deploymenttype=docker
   ...  collect the time it takes to create the clusters and write it to a file
...
# ECQ-2701
ClusterInst shall create 6 with IpAccessShared/docker on openstack
   [Documentation]
   ...  create 6 clusters on openstack with IpAccessShared and deploymenttype=docker
   ...  collect the time it takes to create the clusters and write it to a file
...
# ECQ-2702
ClusterInst shall create 8 with IpAccessShared/docker on openstack
   [Documentation]
   ...  create 8 clusters on openstack with IpAccessShared and deploymenttype=docker
   ...  collect the time it takes to create the clusters and write it to a file
...
# ECQ-2703
ClusterInst shall create 10 with IpAccessShared/docker on openstack
   [Documentation]
   ...  create 10 clusters on openstack with IpAccessShared and deploymenttype=docker
   ...  collect the time it takes to create the clusters and write it to a file
...
Setup
	...
	${FileName}=    Catenate  SEPARATOR=    ${cloudlet_name_openstack}   OpenstackTimingsDockerShared
...
Clean Up
	...
	${epoch_end_time}=     Get Time  epoch
	${epoch_total_time}=   Evaluate    ${epoch_end_time}-${epoch_start_time} 
	${FileData}=     Set Variable       Delete ${testnum} Openstack Docker Shared Cluster Time: ${epoch_total_time}\n
	Append To File   ${EXECDIR}/${FileName}     ${FileData}  
	Write Data   
