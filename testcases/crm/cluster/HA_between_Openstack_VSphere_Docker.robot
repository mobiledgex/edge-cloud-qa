### ECQ-2439 ###
*** Settings ***
Documentation   Create Dedicated Docker Reservable Cluster, Delete Cloudlet from policy and Verify HA works between Opestack and Vsphere
...
Create docker based reservable cluster instnace
   ...
   ${cluster_inst}=  Create Cluster Instance  region=${region}  reservable=${True}   cluster_name=${cluster_name}  cloudlet_name=${cloudlet1}  operator_org_name=${operator_name_openstack_packet}  ip_access=IpAccessDedicated  deployment=docker  flavor_name=${flavor}  developer_org_name=MobiledgeX  token=${super_token}
