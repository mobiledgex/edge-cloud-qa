*** Settings ***
Documentation   Create K8S Reservable Cluster and Verify Auto-Provisioning
...
Create one k8s and one docker based reservable cluster instnace
   ...
   ${cluster_inst}=  Create Cluster Instance  region=${region}  reservable=${True}   cluster_name=${cluster_name}  cloudlet_name=${cloudlet_name_openstack_dedicated}  operator_org_name=${operator_name_openstack}  ip_access=IpAccessDedicated  deployment=kubernetes  flavor_name=${flavor}
...
#   sleep  15s
   ...
   show app instances  region=${region}  developer_org_name=testmonitor  app_version=v1  app_name=${app_name}}  operator_org_name=${operator_name_openstack}  cluster_instance_name=k8sreservable
   log to console  App Instnace is running successfully!

#   sleep  20s

   log to console  delete app instance
   delete app instance  region=${region}  app_name=${app_name}  cluster_instance_name=k8sreservable  cluster_instance_developer_org_name=MobiledgeX  developer_org_name=testmonitor  app_version=v1
