...
<body>
<ul id="myUL">
<li><span class="caret">Cluster Sequential Timings</span><ul class="nested">
'''
...
ttype = ttype[0]
out_html = tname + ttype + 'Sequential.html'
...
with open(tfilename, 'r') as infile:
    ...
    while(inline.find('Delete') == -1):
        if(inline.find(timingdate) != -1):
            inline = infile.readline()
        if((inline.find('Create') != -1) and (inline.find('Delete') == -1)):
