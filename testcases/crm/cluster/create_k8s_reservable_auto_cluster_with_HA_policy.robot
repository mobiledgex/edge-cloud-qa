...
#   ${cluster_inst}=  Create Cluster Instance  region=${region}  reservable=${True}   cluster_name=${cluster_name}  cloudlet_name=${cloudlet_name_openstack_dedicated}  operator_org_name=${operator_name}  ip_access=IpAccessDedicated  deployment=kubernetes  flavor_name=${flavor}  developer_org_name=MobiledgeX  token=${super_token}
...
Create Auto Provisioning Policy
   ...
   &{cloudlet1}=  create dictionary  name=${cloudlet_name_openstack_dedicated}  organization=${operator_name}
...
Create App, Add Autoprovisioning Polivy and Deploy an App Instance
   ...
   sleep  1 minutes
   Wait For App Instance To Be Ready   region=${region}   developer_org_name=${orgname}  app_version=1.0  app_name=${app_name}  cloudlet_name=${cloudlet_name_openstack_dedicated}  operator_org_name=${operator_name}  token=${user_token}  #cluster_instance_name=${cluster_name}
...
Delete app instance and verify auto deployment works again
   [Tags]  ReservableCluster
    ...
    sleep  1 minutes
    Wait For App Instance To Be Ready   region=${region}   developer_org_name=${orgname}  app_version=1.0  app_name=${app_name}  cloudlet_name=${cloudlet_name_openstack_dedicated}  operator_org_name=${operator_name}  token=${user_token}  #cluster_instance_name=${cluster_name}
    ${appInst}=  Show App Instances  region=${region}   developer_org_name=${orgname}  app_version=1.0  app_name=${app_name}  cloudlet_name=${cloudlet_name_openstack_dedicated}  operator_org_name=${operator_name}  token=${user_token}
...
Remove auto provisioning policy from App
    ...
    Wait For App Instance To Be Deleted  app_name=${app_name}  region=${region}  app_version=1.0  developer_org_name=${orgname}
...
Cleanup
     delete cluster instance  region=${region}  cluster_name=${reservable_cluster_name}  developer_org_name=MobiledgeX  cloudlet_name=${cloudlet_name_openstack_dedicated}  operator_org_name=${operator_name}  token=${super_token}
