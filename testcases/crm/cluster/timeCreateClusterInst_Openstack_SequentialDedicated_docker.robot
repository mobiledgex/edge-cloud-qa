...
*** Test Cases ***
# ECQ-2762
Create 5 sequential IpAccessDedicated Docker clusters on openstack
   [Documentation]
   ...  create 5 sequential clusters on openstack with IpAccessDedicated and deploymenttype=docker
   ...  collect the time it takes to create each of the 5 clusters and write it to a file
	...
	${FileData}=     Set Variable       Create Sequential Cluster ${y} Openstack Docker Dedicated Cluster Time: ${epoch_total_time}\n
...
Setup
	...
	${FileName}=    Catenate  SEPARATOR=    ${cloudlet_name_openstack}   OpenstackSequentioalTimingsDockerDedicated
...
Clean Up
	...
	FOR  ${ITEM}   IN   @{clusterList}
	   ...
	   ${epoch_end_time}=     Get Time  epoch
	   ${epoch_total_time}=   Evaluate    ${epoch_end_time}-${epoch_start_time} 
	   ${FileData}=     Set Variable       Delete Sequential Cluster ${b} Openstack Docker Dedicated Cluster Time: ${epoch_total_time}\n
	   Append To File   ${EXECDIR}/${FileName}     ${FileData}
	   ${a}=   Evaluate   ${a}+1
        END
