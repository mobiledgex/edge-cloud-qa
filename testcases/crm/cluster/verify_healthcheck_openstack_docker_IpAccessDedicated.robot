...
*** Test Cases ***
healthcheck shows HealthCheckFailRootlbOffline when docker container is stopped on rootlb for app with one TCP access port
    [Documentation]
...
healthcheck shows HealthCheckFailServerFail when docker container is stopped on clustervm for app with TCP access port
    [Documentation]
    ...  deploy IpAccessShared k8s cluster and app with manifest with shared volume mounts
    ...  verify mounts are persisted over pod restart
...
healthcheck shows HealthCheckFailServerFail when docker container is stopped on clustervm for app with TCP and UDP access port
    [Documentation]
    ...  deploy IpAccessShared k8s cluster and app with manifest with shared volume mounts
    ...  verify mounts are persisted over pod restart
