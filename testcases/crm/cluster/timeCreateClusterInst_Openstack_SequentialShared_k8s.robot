...
*** Variables ***
#${cloudlet_name_openstack}   automationParadiseCloudlet
...
*** Test Cases ***
# ECQ-2765
Create 5 sequential IpAccessShared Kubernetes clusters on openstack
   [Documentation]
   ...  create 5 sequential clusters on openstack with IpAccessShared and deploymenttype=kubernetes
   ...  collect the time it takes to create each of the 5 clusters and write it to a file
	...
	Create Cluster Instance   cluster_name=${cluster_name}   cloudlet_name=${cloudlet_name_openstack}   operator_org_name=${operator_name_openstack}      flavor_name=${flavor_name}   number_nodes=0  number_masters=0   ip_access=IpAccessShared    deployment=kubernetes     
	${epoch_end_time}=     Get Time  epoch
	${epoch_total_time}=   Evaluate    ${epoch_end_time}-${epoch_start_time} 
	${FileData}=     Set Variable       Create Sequential Cluster ${y} Openstack Kubernetes Shared Cluster Time: ${epoch_total_time}\n
...
Setup
	...
	${FileName}=    Catenate  SEPARATOR=    ${cloudlet_name_openstack}   OpenstackSequentioalTimingsK8sShared
...
Clean Up
	...
	FOR  ${ITEM}   IN   @{clusterList}
	   ...
	   ${epoch_end_time}=     Get Time  epoch
	   ${epoch_total_time}=   Evaluate    ${epoch_end_time}-${epoch_start_time} 
	   ${FileData}=     Set Variable       Delete Sequential Cluster ${b} Openstack Kubernetes Shared Cluster Time: ${epoch_total_time}\n
	   Append To File   ${EXECDIR}/${FileName}     ${FileData}
	   ${a}=   Evaluate   ${a}+1
        END
