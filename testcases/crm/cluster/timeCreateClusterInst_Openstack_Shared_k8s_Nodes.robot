...
ClusterInst shall create single with IpAccessShared/kubernetes with 2 Nodes on openstack
   ...
   ...  collect the time it takes to create the cluster and write it to a file
	...
	${epoch_start_time}=   Get Time  epoch
	Create Cluster Instance   cluster_name=${cluster_name}   cloudlet_name=${cloudlet_name_openstack}   operator_org_name=${operator_name_openstack}     flavor_name=${flavor_name}   number_nodes=2  number_masters=1   ip_access=IpAccessShared    deployment=kubernetes     	
	${epoch_end_time}=     Get Time  epoch
	${epoch_total_time}=   Evaluate    ${epoch_end_time}-${epoch_start_time} 
...
ClusterInst shall create single with IpAccessShared/kubernetes with 3 Nodes on openstack
   ...
   ...  collect the time it takes to create the cluster and write it to a file
	...
	${epoch_start_time}=   Get Time  epoch
	Create Cluster Instance   cluster_name=${cluster_name}   cloudlet_name=${cloudlet_name_openstack}   operator_org_name=${operator_name_openstack}      flavor_name=${flavor_name}   number_nodes=3  number_masters=1   ip_access=IpAccessShared    deployment=kubernetes     	
	${epoch_end_time}=     Get Time  epoch
	${epoch_total_time}=   Evaluate    ${epoch_end_time}-${epoch_start_time} 
...
ClusterInst shall create single with IpAccessShared/kubernetes with 4 Nodes on openstack
   ...
   ...  collect the time it takes to create the cluster and write it to a file
	...
	${epoch_start_time}=   Get Time  epoch
	Create Cluster Instance   cluster_name=${cluster_name}   cloudlet_name=${cloudlet_name_openstack}   operator_org_name=${operator_name_openstack}      flavor_name=${flavor_name}   number_nodes=4  number_masters=1   ip_access=IpAccessShared    deployment=kubernetes     	
	${epoch_end_time}=     Get Time  epoch
	${epoch_total_time}=   Evaluate    ${epoch_end_time}-${epoch_start_time} 
...
ClusterInst shall create single with IpAccessShared/kubernetes with 5 Nodes on openstack
   ...
   ...  collect the time it takes to create the cluster and write it to a file
	...
	${epoch_start_time}=   Get Time  epoch
	Create Cluster Instance   cluster_name=${cluster_name}   cloudlet_name=${cloudlet_name_openstack}   operator_org_name=${operator_name_openstack}      flavor_name=${flavor_name}   number_nodes=5  number_masters=1   ip_access=IpAccessShared    deployment=kubernetes     	
	${epoch_end_time}=     Get Time  epoch
	${epoch_total_time}=   Evaluate    ${epoch_end_time}-${epoch_start_time} 
...
ClusterInst shall create single with IpAccessShared/kubernetes with 10 Nodes on openstack
   ...
   ...  collect the time it takes to create the cluster and write it to a file
	...
	${epoch_start_time}=   Get Time  epoch
	Create Cluster Instance   cluster_name=${cluster_name}   cloudlet_name=${cloudlet_name_openstack}   operator_org_name=${operator_name_openstack}      flavor_name=${flavor_name}   number_nodes=10  number_masters=1   ip_access=IpAccessShared    deployment=kubernetes     	
	${epoch_end_time}=     Get Time  epoch
	${epoch_total_time}=   Evaluate    ${epoch_end_time}-${epoch_start_time} 
