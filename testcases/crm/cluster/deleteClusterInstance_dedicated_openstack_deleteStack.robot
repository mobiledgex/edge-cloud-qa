...
ClusterInst/AppInst shall be deleted with IpAccessDedicated/docker on openstack when no stack exists
   ...
   ${cluster_inst}=  Create Cluster Instance  cloudlet_name=${cloudlet_name_openstack_dedicated}  operator_org_name=${operator_name_openstack}  number_nodes=0  number_masters=0  ip_access=IpAccessDedicated  deployment=docker  #no_auto_delete={$True}
