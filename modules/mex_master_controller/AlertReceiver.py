...
class AlertReceiver(MexOperation):
    ...
    def _build(self, region=None, receiver_name=None, type=None, severity=None, email_address=None, user=None, pagerduty_integration_key=None, slack_channel=None, slack_api_url=None, app_name=None, app_version=None, app_cloudlet_name=None, app_cloudlet_org=None, cloudlet_name=None, operator_org_name=None, developer_org_name=None, cluster_instance_name=None, cluster_instance_developer_org_name=None, use_defaults=True):
        ...
        if use_defaults:
            if receiver_name is None:
                receiver_name = shared_variables.alert_receiver_name_default
            if type is None:
                type = shared_variables.alert_receiver_type_default
            if severity is None:
                severity = shared_variables.alert_receiver_severity_default
            ...
            if len(response['messages']) > 0:
                ...
                for slack_msg in response['messages']:
                    ...
                    if alert_type in subject and subject_to_check in subject:
                        ...
                        try:
                            if alert_name:
                                check_payload(slack_msg, f'*alertname:* {alert_name}')
                            if app_name:
                                check_payload(slack_msg, f'*app:* {app_name}')
                            if app_version:
                                check_payload(slack_msg, f'*appver:* {app_version}')
                            if developer_org_name:
                                check_payload(slack_msg, f'*apporg:* {developer_org_name}')
                            if cloudlet_name:
                                check_payload(slack_msg, f'*cloudlet:* {cloudlet_name}')
                            if operator_org_name:
                                check_payload(slack_msg, f'*cloudletorg:* {operator_org_name}')
                            if cluster_instance_name:
                                check_payload(slack_msg, f'*cluster:* {cluster_instance_name}')
                            if cluster_instance_developer_org_name:
                                check_payload(slack_msg, f'*clusterorg:* {cluster_instance_developer_org_name}')
                            if region:
                                check_payload(slack_msg, f'*region:* {region}')
                            if status:
                                check_payload(slack_msg, f'*status:* {status}')
                            if scope:
                                check_payload(slack_msg, f'*scope:* {scope}')
                            if port:
                                check_payload(slack_msg, f'*port:* {port}')
                            if description:
                                check_payload(slack_msg, f'*Description:* {description}')
                            if title:
                                check_payload(slack_msg, f'*Alert:* {title}')
                        ...
                        if alert_type == 'RESOLVED':
                            if slack_msg['attachments'][0]['color'] == '2eb886':
                                logger.info('color 2eb886 found in resolved alert slack message')
                            else:
                                raise Exception('color 2eb886 not found in resolved alert slack message')
                        elif alert_type == 'FIRING':
                            if slack_msg['attachments'][0]['color'] == 'a30200':
                                logger.info('color a30200 found in firing alert slack message')
                            else:
                                raise Exception('color a30200 not found in firing alert slack message')
    ...
    def verify_email(self, email_address, email_password, alert_type, alert_receiver_name, alert_name, status=None, region=None, app_name=None, app_version=None, developer_org_name=None, cloudlet_name=None, operator_org_name=None, cluster_instance_name=None, cluster_instance_developer_org_name=None, port=None, scope=None, description=None, title=None, server='imap.gmail.com', receiver_type='email', pagerduty_status=None, wait=30):
        ...
        for attempt in range(wait):
            ...
            emailstatus, email_list = mail.search(None, email_heading_search)
            # if receiver_type == 'pagerduty':
            #    emailstatus, email_list = mail.search(None, f'(SUBJECT "PagerDuty")')
            # else:
