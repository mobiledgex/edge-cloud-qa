...
printf "However you can still use this script on any cluster if you use the app jmeterk8sapp8086 just change the Jdomain as the port will be the same\n"
...
printf "$normal\t./jmeterbash.sh jmeterx8086.jmx 900 35 5 5 8086 testappalertcluster.automationdallascloudlet.packet.mobiledgex.net  <=== bash values only\n"
...
printf "$normal\t./jmeterbash.sh \$1 \$2 \$3 \$4 \$5 \$6 \$7 <===== customize the 7 arguments as follows\n"
printf "$normal\t./jmeterbash.sh jmeterx8086.jmx 900 35 5 5 8086 testappalertcluster.automationdallascloudlet.packet.mobiledgex.net <=== bash values only\n"
printf "$normal\t./jmeterbash.sh jmeterx8086.jmx 900 35 5 5 8086 somecluster.somecloudlet.packet.mobiledgex.net                  <=== change cluster and dedicatedLB \n"
printf "$normal\t./jmeterbash.sh <jmx file> <delay ms> <seconds/loops> <users> <ramptime> <http port> <http url>  <=== \$1 \$2 \$3 \$4 \$5 \$6 \$7 value info\n"
printf "$green \tjmeter -n -t  $jmjmxfile $Jdelay $Jloops $Jusers $Jramptime $Jport $Jdomain $normal  <==== this is jmeter with the J vars\n"
echo
printf "$yellow This bash file is using jmeter with input args with a modified jmx file to make sending mcctl commands$normal\n"
...
printf "$normal\t./jmeterbash.sh jmeterx8086.jmx 900 35 5 5 8086 testappalertcluster.automationdallascloudlet.packet.mobiledgex.net 30 <=== bash values only\n"
printf "$normal\t./jmeterbash.sh jmeterx8086.jmx 900 35 5 5 8086 somecluster.somecloudlet.packet.mobiledgex.net 30                  <=== change cluster and dedicatedLB \n"
...
printf "$normal\t./jmeterbash.sh jmeterx8086.jmx 900 35 5 5 8086 <=== bash values only\n"
